...
def metanode_9621(port_01, port_02, port_03):
    ...
    # CCU [Mt] 
    CCU_Mt = UseVariableNode(selected_variable='CCU[Mt]')(input_table=port_02)
    ...
    direct_air_capture_Mt = UseVariableNode(selected_variable='direct-air-capture[Mt]')(input_table=port_03)
    # Group by  Country, Years, gaes (SUM)
    direct_air_capture_Mt = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'gaes'], aggregation_method='Sum')(df=direct_air_capture_Mt)
    # remaining-CCU[Mt] (correspond to remaining CCU after DAC) = CCU[Mt] - direct-air-capture[Mt]
    remaining_CCU_Mt = MCDNode(operation_selection='y - x', output_name='remaining-CCU[Mt]')(input_table_1=direct_air_capture_Mt, input_table_2=CCU_Mt)
    # CC [Mt] from Power and other sectors
    CC_Mt = UseVariableNode(selected_variable='CC[Mt]')(input_table=port_01)
    # Group by  Country, Years, sector, gaes (SUM)
    CC_Mt = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'gaes', 'sector'], aggregation_method='Sum')(df=CC_Mt)
    # Group by  Country, Years, gaes (SUM)
    CC_Mt_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'gaes'], aggregation_method='Sum')(df=CC_Mt)
    # CC-proportion[-] = CC[Mt] (by sector) / CC[Mt] (total)
    CC_proportion = MCDNode(operation_selection='x / y', output_name='CC-proportion[-]')(input_table_1=CC_Mt, input_table_2=CC_Mt_2)
    ...
    CC_proportion_excluded = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=CC_proportion_excluded)
    # Set 1 (no CC) (refineries)
    CC_proportion = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='1.0')(df=CC_proportion_2)
    ...
    CCS_Mt = MCDNode(operation_selection='x - y', output_name='CCS[Mt]')(input_table_1=CC_Mt_2, input_table_2=remaining_CCU_Mt)
    # CCS[Mt] (replace) (detailled by sector) = CCS[Mt] * CC-proportion[-]
    CCS_Mt = MCDNode(operation_selection='x * y', output_name='CCS[Mt]')(input_table_1=CC_proportion, input_table_2=CCS_Mt)
    ...
    out_7313_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'gaes', 'sector'], aggregation_method='Sum')(df=out_7313_1)
    ...
    # CCS [Mt]
    CCS_Mt = UseVariableNode(selected_variable='CCS[Mt]')(input_table=out_7313_1)
