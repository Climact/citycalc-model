...
def metanode_6514(port_01, port_02, port_03, port_04):
    ...
    # Keep only sector heat
    port_04 = port_04.loc[port_04['sector'].isin(['heat'])].copy()
    ...
    exogenous_energy_costs_MEUR_per_TWh = exogenous_energy_costs_MEUR_per_TWh.loc[exogenous_energy_costs_MEUR_per_TWh['energy-carrier'].isin(['heat'])].copy()
    # Group by  Country, Years (MEAN)
    exogenous_energy_costs_MEUR_per_TWh_2 = GroupByDimensions(groupby_dimensions=['Years', 'Country'], aggregation_method='Mean')(df=exogenous_energy_costs_MEUR_per_TWh)

    # Calibration

    # Apply Calibration on fuel-price[MEUR/TWh]
    fuel_price_endogenous_MEUR_per_TWh, _, _ = CalibrationNode(data_to_be_cal='fuel-price-endogenous[MEUR/TWh]', data_cal='exogenous-energy-costs[EUR/MWh]')(input_table=fuel_price_endogenous_MEUR_per_TWh, cal_table=exogenous_energy_costs_MEUR_per_TWh_2)
