...
def metanode_9096(port_01):
    ...
    out_9540_1 = TableCreatorNode(df=pd.DataFrame(columns=['category-from', 'energy-carrier-from', 'category-to', 'energy-carrier-to'], data=[['fffuels', 'liquid-ff-diesel', 'biofuels', 'liquid-bio-diesel'], ['fffuels', 'liquid-ff-gasoline', 'biofuels', 'liquid-bio-gasoline'], ['fffuels', 'gaseous-ff-natural', 'biofuels', 'gaseous-bio'], ['fffuels', 'liquid-ff-kerosene', 'biofuels', 'liquid-bio-kerosene'], ['fffuels', 'liquid-ff-marinefueloil', 'biofuels', 'liquid-bio-marinefueloil'], ['fffuels', 'liquid-ff-diesel', 'synfuels', 'liquid-syn-diesel'], ['fffuels', 'liquid-ff-gasoline', 'synfuels', 'liquid-syn-gasoline'], ['fffuels', 'gaseous-ff-natural', 'synfuels', 'gaseous-syn'], ['fffuels', 'liquid-ff-kerosene', 'synfuels', 'liquid-syn-kerosene'], ['fffuels', 'liquid-ff-marinefueloil', 'synfuels', 'liquid-syn-marinefueloil']]))()
    ...
    out_9472_1 = TableCreatorNode(df=pd.DataFrame(columns=['category-from', 'motor-type-from', 'category-to', 'motor-type-to'], data=[['thermic', 'ICE', 'electric', 'BEVnew']]))()
    # ratio[-] = 1
    ...
    out_5173_1 = TableCreatorNode(df=pd.DataFrame(columns=['mult-factor', 'material'], data=[[0.05, 'roads'], [0.05, 'rails'], [0.1, 'trolley-cables']]))()

    # Hard codé ici ==> à terme, introduire dans une google sheet !

    # Correlation for material
    out_5133_1 = TableCreatorNode(df=pd.DataFrame(columns=['vehicule-type', 'material'], data=[['LDV', 'roads'], ['HDV', 'roads'], ['bus', 'roads'], ['rail', 'rails']]))()
    ...
    distance_traveled_pkm = UseVariableNode(selected_variable='distance-traveled[pkm]')(input_table=out_5965_1)
    # (LFS) transport-demand aviation (pkm)
    transport_demand_pkm_2 = UseVariableNode(selected_variable='transport-demand[pkm]')(input_table=out_5965_1)
    ...
    vehicle_lifetime_km_km = ImportDataNode(trigram='tra', variable_name='vehicle-lifetime-km', variable_type='OTS (only)')()
    # Same as last available year
    vehicle_lifetime_km_km = AddMissingYearsNode()(df_data=vehicle_lifetime_km_km)
    # Group by  transport-user, vehicule-type (max)
    vehicle_lifetime_km_km = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Maximum')(df=vehicle_lifetime_km_km)

    # Apply utilization-rate levers (improve)
    # => Determine the % of use of a vehicle (for non-public vehicle)

    # OTS / FTS utilization-rate
    utilization_rate = ImportDataNode(trigram='tra', variable_name='utilization-rate')()
    # renewal-rate[%] = utilization-rate / lifetime
    renewal_rate_percent = MCDNode(operation_selection='y / x', output_name='renewal-rate[%]')(input_table_1=vehicle_lifetime_km_km, input_table_2=utilization_rate)
    ...
    renewal_rate_percent_2 = ImportDataNode(trigram='tra', variable_name='renewal-rate', variable_type='OTS (only)')()
    # Same as last available year
    renewal_rate_percent_2 = AddMissingYearsNode()(df_data=renewal_rate_percent_2)
    ...
    # renewal-rate [%]
    renewal_rate_percent_2 = UseVariableNode(selected_variable='renewal-rate[%]')(input_table=renewal_rate_percent)
    ...
    renewal_rate_percent_3 = ColumnFilterNode(columns_to_drop=[])(df=renewal_rate_percent)
    ...
    out_6009_1, _, _ = PivotingNode(agg_dict={'renewal-rate[%]': 'sum'}, column_name_option='Pivot name+Aggregation name', column_name_policy='Keep original name(s)', list_group_columns=['Years', 'Country'], list_pivots=['transport-user', 'vehicule-type'])(df=out_6019_1)
    out_6002_1 = MissingValueColumnFilterNode(missing_threshold=0.9, type_of_pattern='Manual')(df=out_6009_1)
    # Remove Aggregation Name
    out_6002_1 = ColumnRenameRegexNode(search_string='(.*)\\+.*', replace_string='$1')(df=out_6002_1)
    # Same as in Pathway Explorer
    out_6002_1 = ColumnRenameRegexNode(search_string='(.*)_(.*)', replace_string='tra_$1_renewal-rate_$2[%]')(df=out_6002_1)
    # Filter out aviation + WW +  marine + rail  + metro-tram
    out_6002_1 = ColumnFilterNode(pattern='^((?!.*aviation|IWW|marine|rail|metro-tram.*).)*$')(df=out_6002_1)

    # Add dummy columns (not modelized in XCalc)

    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_6004_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_freight_renewal-rate_HDVM[%]', column_type='DOUBLE')(df=out_6002_1)

    # Renewal-need =
    # 1-(1-$$CURRENT_COLUMN$$)^$Timestep$

    # Inner join
    out_4997_1 = JoinerNode(joiner='inner', left_input=['Years'], right_input=['Years'])(df_left=renewal_rate_percent, df_right=out_4996_1)
    ...
    # renewal-need [%]
    renewal_need_percent = ExportVariableNode(selected_variable='renewal-need[%]')(input_table=renewal_need_percent)
    # renewal-need [%]
    renewal_need_percent = UseVariableNode(selected_variable='renewal-need[%]')(input_table=renewal_need_percent)
    ...
    national_share_percent = ImportDataNode(trigram='tra', variable_name='national-share', variable_type='OTS (only)')()
    # Same as last available year
    national_share_percent = AddMissingYearsNode()(df_data=national_share_percent)
    # OTS (only) avg-number-vehicles-tkm [veh/tkm]
    avg_number_vehicles_tkm_veh_per_tkm = ImportDataNode(trigram='tra', variable_name='avg-number-vehicles-tkm', variable_type='OTS (only)')()
    # Same as last available year
    avg_number_vehicles_tkm_veh_per_tkm = AddMissingYearsNode()(df_data=avg_number_vehicles_tkm_veh_per_tkm)
    ...
    avg_number_vehicles_pkm_veh_per_bn_pkm = ImportDataNode(trigram='tra', variable_name='avg-number-vehicles-pkm', variable_type='OTS (only)')()
    # Same as last available year
    avg_number_vehicles_pkm_veh_per_bn_pkm = AddMissingYearsNode()(df_data=avg_number_vehicles_pkm_veh_per_bn_pkm)
    ...
    demand_share_inland_percent = ImportDataNode(trigram='tra', variable_name='demand-share-inland')()
    ...
    international_freight_demand_bn_tkm = ImportDataNode(trigram='tra', variable_name='international-freight-demand')()

    # Modal-share levels are applied later in the flow

    # OTS/FTS modal-share [%]
    modal_share_percent = ImportDataNode(trigram='tra', variable_name='modal-share')()

    # Calculation for post-calibration

    # keep only OTS (years <= baseyear)
    modal_share_percent_2, _ = FilterDimension(dimension='Years', operation_selection='≤', value_years=Globals.get().base_year)(df=modal_share_percent)
    # Same as last available year
    modal_share_percent_2 = AddMissingYearsNode()(df_data=modal_share_percent_2)
    # transport-demand[pkm] = modal-share[%]  * distance-traveled[pkm]
    transport_demand_pkm = MCDNode(operation_selection='x * y', output_name='transport-demand[pkm]')(input_table_1=modal_share_percent_2, input_table_2=distance_traveled_pkm)

    # Transport-demand
    # By Country/Years/Vehicule-type

    # Group by  transport-user, vehicule-type (sum)
    transport_demand_pkm = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_pkm)
    ...
    # transport-demand[pkm] = modal-share[%]  * distance-traveled[pkm]
    transport_demand_pkm = MCDNode(operation_selection='x * y', output_name='transport-demand[pkm]')(input_table_1=modal_share_percent, input_table_2=distance_traveled_pkm)
    # Group by  transport-user, vehicule-type (sum)
    transport_demand_pkm_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_pkm)
    ...
    transport_demand_pkm_pkm = ImportDataNode(trigram='tra', variable_name='transport-demand-pkm', variable_type='Calibration')()
    # Apply Calibration on transport-demand[pkm]
    transport_demand_pkm_2, _, out_7099_3 = CalibrationNode(data_to_be_cal='transport-demand[pkm]', data_cal='transport-demand[pkm]')(input_table=transport_demand_pkm_2, cal_table=transport_demand_pkm_pkm)
    ...
    cal_rate_transport_demand_pkm = UseVariableNode(selected_variable='cal_rate_transport-demand[pkm]')(input_table=out_7099_3)
    ...
    transport_demand_pkm_excluded_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=transport_demand_pkm_excluded)
    # Apply Calibration on transport-demand[pkm]
    transport_demand_pkm_3, _, _ = CalibrationNode(data_to_be_cal='transport-demand[pkm]', data_cal='transport-demand[pkm]')(input_table=transport_demand_pkm_3, cal_table=transport_demand_pkm_pkm)
    # bottom : all except aviation
    ...
    # Group by  transport-user (sum)
    transport_demand_pkm_excluded_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=transport_demand_pkm_excluded_3)
    # Calculate  Post-Calibration factors
    _, out_9522_2, _ = CalibrationNode(data_to_be_cal='transport-demand[pkm]', data_cal='transport-demand[pkm]')(input_table=transport_demand_pkm_excluded_2, cal_table=transport_demand_pkm_excluded_3)
    # transport-demand[pkm] (replace) = transport-demand[pkm]  * cal_rate (post-calibration)
    transport_demand_pkm_3 = MCDNode(operation_selection='x * y', output_name='transport-demand[pkm]')(input_table_1=transport_demand_pkm_excluded, input_table_2=out_9522_2)
    ...
    transport_demand_pkm_3 = ExportVariableNode(selected_variable='transport-demand[pkm]')(input_table=transport_demand_pkm_2)

    # Select variables from transport demand

    # transport-demand [pkm]
    transport_demand_pkm_2 = UseVariableNode(selected_variable='transport-demand[pkm]')(input_table=transport_demand_pkm_3)
    ...
    veh_fleet_total_number_2 = MCDNode(operation_selection='x * y', output_name='veh-fleet-total[number]')(input_table_1=transport_demand_bn_pkm, input_table_2=avg_number_vehicles_pkm_veh_per_bn_pkm)
    # Pins (all selected)
    transport_demand_pkm_3 = ColumnFilterNode(pattern='^.*$')(df=transport_demand_pkm_3)
    ...
    # transport-demand [pkm]
    transport_demand_pkm_3 = UseVariableNode(selected_variable='transport-demand[pkm]')(input_table=transport_demand_pkm_3)

    # Modal share of LDV passenger

    # transport-demand [pkm] (including bike and walk)
    transport_demand_pkm_4 = UseVariableNode(selected_variable='transport-demand[pkm]')(input_table=transport_demand_pkm_3)
    ...
    # Group by  year
    transport_demand_pkm_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=transport_demand_pkm_5)
    # modal-share-passenger-LDV[pkm%] = transport-demand-per-cap (LDV) [pkm/cap] / transport-demand-per-cap (all) [pkm/cap]
    modal_share_passenger_LDV_percent_pkm = MCDNode(operation_selection='x / y', output_name='modal-share-passenger-LDV[%pkm]')(input_table_1=transport_demand_pkm_6, input_table_2=transport_demand_pkm_5)
    # modal-share-passenger-LDV[pkm%] 
    modal_share_passenger_LDV_percent_pkm = ExportVariableNode(selected_variable='modal-share-passenger-LDV[%pkm]')(input_table=modal_share_passenger_LDV_percent_pkm)

    # For : Pathway Explorer

    # Group by  dimensions
    transport_demand_pkm_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_pkm_4)

    # Apply occupancy levers (avoid)
    # => determine the nb of passenger per mode

    # OTS/FTS occupancy [pkm/vkm]
    occupancy_pkm_per_vkm = ImportDataNode(trigram='tra', variable_name='occupancy')()
    ...
    automation_add_empty_km_percent = ImportDataNode(trigram='tra', variable_name='automation-add-empty-km')()
    # OTS/FTS automation-share [%]
    automation_share_percent = ImportDataNode(trigram='tra', variable_name='automation-share')()
    # automation-factor[%] = 1 + automation-add-empty-km[%] * automation-share[%]
    automation_factor_percent = MCDNode(operation_selection='1 + x * y', output_name='automation-factor[%]')(input_table_1=automation_add_empty_km_percent, input_table_2=automation_share_percent)

    # For : ALL RESULTS (allows visualization outside the module node)

    # Group by  transport-user (sum)
    transport_demand_pkm_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=transport_demand_pkm_2)
    # Add total prefix
    transport_demand_pkm_5 = ColumnRenameRegexNode(search_string='(.*\\[.*)', replace_string='total-$1')(df=transport_demand_pkm_5)
    # modal-share[%] = transport-demand[pkm] / total-transport-demand[pkm]
    modal_share_percent_2 = MCDNode(operation_selection='y / x', output_name='modal-share[%]')(input_table_1=transport_demand_pkm_5, input_table_2=transport_demand_pkm_2)
    # veh-transport-demand[vkm] = transport-demand[pkm] / occupancy[pkm/vkm]
    veh_transport_demand_vkm = MCDNode(operation_selection='x / y', output_name='veh-transport-demand[vkm]')(input_table_1=transport_demand_pkm_2, input_table_2=occupancy_pkm_per_vkm)
    # veh-transport-demand[vkm] = (replace) veh-transport-demand[vkm] x automation-factor[%]  Fill missing factor with 1
    veh_transport_demand_vkm_2 = MCDNode(operation_selection='x * y', output_name='veh-transport-demand[vkm]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=veh_transport_demand_vkm, input_table_2=automation_factor_percent)
    # Pins (all selected)
    veh_transport_demand_vkm = ColumnFilterNode(pattern='^.*$')(df=veh_transport_demand_vkm_2)
    # veh-transport-demand [vkm] from pkm
    veh_transport_demand_vkm = UseVariableNode(selected_variable='veh-transport-demand[vkm]')(input_table=veh_transport_demand_vkm)

    # Apply utilization-rate levers (improve)
    # => Determine the % of use of a vehicle (for non-public vehicle)

    # veh-fleet-total[number] = veh-transport-demand[vkm] / utilization-rate[vkm/veh]
    veh_fleet_total_number = MCDNode(operation_selection='x / y', output_name='veh-fleet-total[number]')(input_table_1=veh_transport_demand_vkm_2, input_table_2=utilization_rate)
    ...
    # transport-demand[pkm]
    transport_demand_pkm_5 = UseVariableNode(selected_variable='transport-demand[pkm]')(input_table=transport_demand_pkm)

    # Apply occupancy levers (avoid)
    # => determine the nb of passenger per mode

    # veh-transport-demand[vkm] = transport-demand[pkm] / occupancy[pkm/vkm]
    veh_transport_demand_vkm_2 = MCDNode(operation_selection='x / y', output_name='veh-transport-demand[vkm]')(input_table_1=transport_demand_pkm_5, input_table_2=occupancy_pkm_per_vkm)
    # Top: only urban
    ...
    veh_transport_demand_vkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=veh_transport_demand_vkm_3)
    # Remove distance-type
    veh_transport_demand_vkm_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=veh_transport_demand_vkm_2)
    # urban-share[%vkm] = transport-demand[vkm] (only urban) / transport-demand[vkm ] (total)
    urban_share_percent_vkm = MCDNode(operation_selection='x / y', output_name='urban-share[%vkm]')(input_table_1=veh_transport_demand_vkm_3, input_table_2=veh_transport_demand_vkm_2)
    # Remove distance-type
    transport_demand_pkm = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_pkm_5)
    # Top: only urban
    transport_demand_pkm_5 = transport_demand_pkm_5.loc[transport_demand_pkm_5['distance-type'].isin(['urban'])].copy()
    # Remove distance-type
    transport_demand_pkm_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_pkm_5)
    # urban-share[%pkm] = transport-demand[pkm] (only urban) / transport-demand[pkm ] (total)
    urban_share_percent_pkm = MCDNode(operation_selection='x / y', output_name='urban-share[%pkm]')(input_table_1=transport_demand_pkm_5, input_table_2=transport_demand_pkm)
    # transport-demand-urban[pkm] = urban-share[%pkm] * transport-demand[pkm] (total)
    transport_demand_urban_pkm = MCDNode(operation_selection='x * y', output_name='transport-demand-urban[pkm]')(input_table_1=urban_share_percent_pkm, input_table_2=transport_demand_pkm_2)
    # transport-demand-urban[pkm]
    transport_demand_urban_pkm_2 = ExportVariableNode(selected_variable='transport-demand-urban[pkm]')(input_table=transport_demand_urban_pkm)
    # GroupBy transport-user
    transport_demand_urban_pkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=transport_demand_urban_pkm)
    # urban-transport-share[%pkm] = transport-demand-urban[pkm] (per veh-type) / transport-demand-urban[pkm] (total)
    urban_transport_share_percent_pkm = MCDNode(operation_selection='x / y', output_name='urban-transport-share[%pkm]')(input_table_1=transport_demand_urban_pkm, input_table_2=transport_demand_urban_pkm_3)
    # urban-transport-share[%pkm]
    urban_transport_share_percent_pkm = ExportVariableNode(selected_variable='urban-transport-share[%pkm]')(input_table=urban_transport_share_percent_pkm)
    # KPI
    ...
    technology_share_new_percent = ImportDataNode(trigram='tra', variable_name='technology-share-new')()
    ...
    technology_share_tot_percent = ExportVariableNode(selected_variable='technology-share-tot[%]')(input_table=out_4985_1)

    # Apply technology-share-mix levers (improve)
    # => Determine the share of technology

    # OTS / FTS technology-share-mix [%]
    technology_share_mix_percent = ImportDataNode(trigram='tra', variable_name='technology-share-mix')()
    # Same as last available year (some of the dimensions are OTS only and need to be completed up to 2050)
    technology_share_mix_percent = AddMissingYearsNode()(df_data=technology_share_mix_percent)
    # technology-share[%] = technology-share-tot[%] x technology-share-mix[%]  Fill techno-share-mix missing with 1
    technology_share_percent = MCDNode(operation_selection='x * y', output_name='technology-share[%]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=technology_share_tot_percent, input_table_2=technology_share_mix_percent)
    ...
    # technology-share [%]
    technology_share_percent = ExportVariableNode(selected_variable='technology-share[%]')(input_table=out_5210_1)

    # Select variables

    # technology-share [%]
    technology_share_percent = UseVariableNode(selected_variable='technology-share[%]')(input_table=technology_share_percent)
    # Only keep ZEV
    ...
    technology_share_percent_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'mix-type', 'fleet-age'], aggregation_method='Sum')(df=technology_share_percent_2)
    # technology-share [%] to  technology-share-new [%]
    technology_share_percent_2 = ColumnRenameRegexNode(search_string='technology-share', replace_string='technology-share-new')(df=technology_share_percent_2)
    # technology-share-new [%]
    technology_share_new_percent = ExportVariableNode(selected_variable='technology-share-new[%]')(input_table=technology_share_percent_2)
    # OTS (only) technology-share-fleet [%]
    technology_share_fleet_percent = ImportDataNode(trigram='tra', variable_name='technology-share-fleet', variable_type='OTS (only)')()
    # Same as last available year
    technology_share_fleet_percent = AddMissingYearsNode()(df_data=technology_share_fleet_percent)

    # Values for Years <= baseyear

    # technology-share-fleet [%]
    technology_share_fleet_percent = UseVariableNode(selected_variable='technology-share-fleet[%]')(input_table=technology_share_fleet_percent)
    # technology-share-fleet[%] to technology-share[%]
    technology_share_fleet_percent_2 = ColumnRenameRegexNode(search_string='(technology-share)-fleet(\\[.*)', replace_string='$1$2')(df=technology_share_fleet_percent)
    ...
    demand_inland_tkm_bn_tkm = ImportDataNode(trigram='tra', variable_name='demand-inland-tkm')()
    # distance-traveled[bn_tkm] = demand-inland[bn_tkm] x demand-share-inland[%]
    distance_traveled_bn_tkm = MCDNode(operation_selection='x * y', output_name='distance-traveled[bn_tkm]')(input_table_1=demand_inland_tkm_bn_tkm, input_table_2=demand_share_inland_percent)
    ...
    transport_demand_bn_tkm = MCDNode(operation_selection='x * y', output_name='transport-demand[bn_tkm]')(input_table_1=modal_share_percent, input_table_2=distance_traveled_bn_tkm)
    # Group by  transport-user, vehicule-type (sum)
    transport_demand_bn_tkm = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_bn_tkm)
    ...
    transport_demand_tkm_bn_tkm = ImportDataNode(trigram='tra', variable_name='transport-demand-tkm', variable_type='Calibration')()
    # Apply Calibration on transport-demand[Gtkm]
    _, out_7100_2, out_7100_3 = CalibrationNode(data_to_be_cal='transport-demand[bn_tkm]', data_cal='transport-demand[bn_tkm]')(input_table=demand_bn_tkm, cal_table=transport_demand_tkm_bn_tkm)
    ...
    cal_rate_transport_demand_bn_tkm = UseVariableNode(selected_variable='cal_rate_transport-demand[bn_tkm]')(input_table=out_9233_1)
    ...
    transport_demand_bn_tkm = MCDNode(operation_selection='x * y', output_name='transport-demand[bn_tkm]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=demand_bn_tkm, input_table_2=out_5779_1)
    ...
    transport_demand_tkm_2 = ExportVariableNode(selected_variable='transport-demand[tkm]')(input_table=transport_demand_tkm)
    # Pins (all selected)
    transport_demand_tkm = ColumnFilterNode(pattern='^.*$')(df=transport_demand_tkm_2)
    # transport-demand [tkm]
    transport_demand_tkm = UseVariableNode(selected_variable='transport-demand[tkm]')(input_table=transport_demand_tkm)

    # Select variables

    # transport-demand[tkm]
    transport_demand_tkm_3 = UseVariableNode(selected_variable='transport-demand[tkm]')(input_table=transport_demand_tkm)
    # Group by  transport-user, vehicule-type
    transport_demand_tkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=transport_demand_tkm_3)
    ...
    load_factor_tkm_per_vkm = ImportDataNode(trigram='tra', variable_name='load-factor')()

    # Select variables

    # transport-demand [tkm]
    transport_demand_tkm_2 = UseVariableNode(selected_variable='transport-demand[tkm]')(input_table=transport_demand_tkm_2)
    # Group by  transport-user (sum)
    transport_demand_tkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=transport_demand_tkm_2)
    # Add total prefix
    transport_demand_tkm_3 = ColumnRenameRegexNode(search_string='(.*\\[.*)', replace_string='total-$1')(df=transport_demand_tkm_3)
    # modal-share[%] = transport-demand[tkm] / total-transport-demand[tkm]
    modal_share_percent = MCDNode(operation_selection='y / x', output_name='modal-share[%]')(input_table_1=transport_demand_tkm_3, input_table_2=transport_demand_tkm_2)
    ...
    # modal-share[%]
    modal_share_percent = ExportVariableNode(selected_variable='modal-share[%]')(input_table=modal_share_percent)
    # Group by  dimensions
    modal_share_percent = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type'], aggregation_method='Sum')(df=modal_share_percent)
    # veh-transport-demand[vkm] = transport-demand[tkm] / load-factor[tkm/vkm]  Missing load-factor : set to 1
    veh_transport_demand_vkm_2 = MCDNode(operation_selection='x / y', output_name='veh-transport-demand[vkm]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=transport_demand_tkm_2, input_table_2=load_factor_tkm_per_vkm)
    # veh-fleet-total[number] = veh-transport-demand[vkm] / utilization-rate[vkm/veh]
    veh_fleet_total_number_2 = MCDNode(operation_selection='y / x', output_name='veh-fleet-total[number]')(input_table_1=utilization_rate, input_table_2=veh_transport_demand_vkm_2)
    # veh-fleet-total[number] = veh-transport-demand[tkm] * avg-number-vehicles-tkm[veh/tkm]
    veh_fleet_total_number_3 = MCDNode(operation_selection='x * y', output_name='veh-fleet-total[number]')(input_table_1=veh_transport_demand_vkm_2, input_table_2=avg_number_vehicles_tkm_veh_per_tkm)
    # veh-fleet-total[number] = (replace) total[number] x national-share  Missing value national-share set to 1
    veh_fleet_total_number_3 = MCDNode(operation_selection='x * y', output_name='veh-fleet-total[number]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=veh_fleet_total_number_3, input_table_2=national_share_percent)
    ...
    vehicle_fleet_historical_number = ImportDataNode(trigram='tra', variable_name='vehicle-fleet-historical', variable_type='Calibration')()
    ...
    veh_fleet_total_number = ExportVariableNode(selected_variable='veh-fleet-total[number]')(input_table=veh_fleet_total_number)

    # Calibration

    # Apply Calibration on transport-demand[pkm]
    veh_fleet_total_number, _, _ = CalibrationNode(data_to_be_cal='veh-fleet-total[number]', data_cal='vehicle-fleet-historical[number]')(input_table=veh_fleet_total_number, cal_table=vehicle_fleet_historical_number)
    # veh-fleet-total [number]
    veh_fleet_total_number = ExportVariableNode(selected_variable='veh-fleet-total[number]')(input_table=veh_fleet_total_number)
    # Lag  veh-fleet-total[number]
    out_7157_1, _ = LagVariable(in_var='veh-fleet-total[number]')(df=veh_fleet_total_number)
    # veh-fleet-total-lagged [number]
    veh_fleet_total_lagged_number = UseVariableNode(selected_variable='veh-fleet-total_lagged[number]')(input_table=out_7157_1)
    # veh-fleet-renewal[number] = veh-fleet-total-lagged[number] * renewal-need[%]
    veh_fleet_renewal_number = MCDNode(operation_selection='x * y', output_name='veh-fleet-renewal[number]')(input_table_1=veh_fleet_total_lagged_number, input_table_2=renewal_need_percent)
    # veh-fleet-remaining[number] = veh-fleet-total-lagged[number] - veh-fleet-renewal[number]
    veh_fleet_remaining_number = MCDNode(operation_selection='x - y', output_name='veh-fleet-remaining[number]')(input_table_1=veh_fleet_total_lagged_number, input_table_2=veh_fleet_renewal_number)
    # veh-fleet-total [number]
    veh_fleet_total_number = UseVariableNode(selected_variable='veh-fleet-total[number]')(input_table=veh_fleet_total_number)
    # veh-fleet-historical[number] = veh-fleet-total[number] x technology-share-fleet[%]
    veh_fleet_historical_number = MCDNode(operation_selection='x * y', output_name='veh-fleet-historical[number]')(input_table_1=technology_share_fleet_percent, input_table_2=veh_fleet_total_number)
    ...
    veh_fleet_historical_number = ExportVariableNode(selected_variable='veh-fleet-historical[number]')(input_table=veh_fleet_historical_number)
    ...
    veh_efficiency_tkm_MJ_per_tkm = ImportDataNode(trigram='tra', variable_name='veh-efficiency-tkm')()
    # Keep new
    ...
    veh_efficiency_pkm_MJ_per_pkm = ImportDataNode(trigram='tra', variable_name='veh-efficiency-pkm')()
    # Keep new
    ...
    veh_efficiency_km_MJ_per_km = ImportDataNode(trigram='tra', variable_name='veh-efficiency-km')()
    # Keep new
    ...
    # Drop fleet-age
    veh_efficiency_MJ_per_excluded = ColumnFilterNode(columns_to_drop=['fleet-age'])(df=veh_efficiency_MJ_per_excluded)
    # Add fleet suffix to veh-eff
    veh_efficiency_MJ_per_excluded = ColumnRenameRegexNode(search_string='(veh-eff.*)', replace_string='fleet-$1')(df=veh_efficiency_MJ_per_excluded)
    ...
    # Drop fleet-age
    veh_efficiency_MJ_per = ColumnFilterNode(columns_to_drop=['fleet-age'])(df=veh_efficiency_MJ_per)
    # Add new suffix to veh-eff
    veh_efficiency_MJ_per = ColumnRenameRegexNode(search_string='(veh-eff.*)', replace_string='new-$1')(df=veh_efficiency_MJ_per)
    # Outer Join : get fleet and new veh-efficiency values
    out_5365_1 = JoinerNode(joiner='outer', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=veh_efficiency_MJ_per, df_right=veh_efficiency_MJ_per_excluded)

    # Average new vehicules GHG emissions (Passenger: LDV, 2W and BUS, Freight: HDV)

    # veh-efficiency-km[MJ/km]
    veh_efficiency_km_MJ_per_km = UseVariableNode(selected_variable='veh-efficiency-km[MJ/km]')(input_table=veh_efficiency_km_MJ_per_km)
    # veh-efficiency-km-avg[MJ/km] = veh-efficiency-km[MJ/km] * technology-share [%]
    veh_efficiency_km_avg_MJ_per_km = MCDNode(operation_selection='x * y', output_name='veh-efficiency-km-avg[MJ/km]')(input_table_1=veh_efficiency_km_MJ_per_km, input_table_2=technology_share_percent)
    # FOR INDUSTRY : veh-fleet-renewal-ind[number] = veh-fleet[number] * renewal-rate[%]
    veh_fleet_renewal_ind_number = MCDNode(operation_selection='x * y', output_name='veh-fleet-renewal-ind[number]')(input_table_1=renewal_rate_percent_2, input_table_2=veh_fleet_total_number)
    # veh-fleet-need[number] = veh-fleet-total[number] - veh-fleet-total-lagged[number]
    veh_fleet_need_number = MCDNode(operation_selection='x - y', output_name='veh-fleet-need[number]')(input_table_1=veh_fleet_total_number, input_table_2=veh_fleet_total_lagged_number)
    ...
    new_veh_fleet_ind_number = MCDNode(operation_selection='x + y', output_name='new-veh-fleet-ind[number]', fill_value_bool='Inner Join')(input_table_1=veh_fleet_renewal_ind_number, input_table_2=out_5286_1)
    # set to 0
    new_veh_fleet_ind_number = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=new_veh_fleet_ind_number)
    # new-veh-fleet-ind [number]
    new_veh_fleet_ind_number = UseVariableNode(selected_variable='new-veh-fleet-ind[number]')(input_table=new_veh_fleet_ind_number)
    # final-veh-fleet-ind[number] = new-veh-fleet-ind[number] x technology-share[%] sent to IND
    final_veh_fleet_ind_number = MCDNode(operation_selection='x * y', output_name='final-veh-fleet-ind[number]')(input_table_1=new_veh_fleet_ind_number, input_table_2=technology_share_percent)
    # Group by  vehicule-type, motor-type, energy-carrier (sum)
    final_veh_fleet_ind_number_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=final_veh_fleet_ind_number)

    # Costs

    # final-veh-fleet-ind [number]
    final_veh_fleet_ind_number_2 = UseVariableNode(selected_variable='final-veh-fleet-ind[number]')(input_table=final_veh_fleet_ind_number_2)
    # RCP costs-by-vehicle [MEUR/number] From TECH
    costs_by_vehicle_MEUR_per_number = ImportDataNode(trigram='tec', variable_name='costs-by-vehicle', variable_type='RCP')()
    ...
    # final-veh-fleet-ind[number]
    final_veh_fleet_ind_number = UseVariableNode(selected_variable='final-veh-fleet-ind[number]')(input_table=final_veh_fleet_ind_number)
    ...
    out_5410_1_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'motor-type'], aggregation_method='Sum')(df=out_5410_1)

    # For : Minerals

    # Keep all
    out_5410_1 = ColumnFilterNode(pattern='^.*$')(df=out_5410_1)
    ...
    # Group by  vehicule-type, motor-type (sum)
    out_5457_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'motor-type'], aggregation_method='Sum')(df=out_5457_1)

    # Pivot

    out_5459_1, _, _ = PivotingNode(agg_dict={'final-veh-fleet-ind[number]': 'sum'}, column_name_option='Pivot name+Aggregation name', column_name_policy='Keep original name(s)', list_group_columns=['Years', 'Country'], list_pivots=['vehicule-type', 'motor-type'])(df=out_5457_1)
    out_5460_1 = MissingValueColumnFilterNode(missing_threshold=0.9, type_of_pattern='Manual')(df=out_5459_1)
    # Remove Aggregation Name
    out_5460_1 = ColumnRenameRegexNode(search_string='(.*)\\+.*', replace_string='$1')(df=out_5460_1)
    # Same as in Pathway Explorer
    out_5460_1 = ColumnRenameRegexNode(search_string='(.*)_(.*)', replace_string='tra_$1_$2[number]')(df=out_5460_1)
    # BEV to EV
    out_5460_1 = ColumnRenameRegexNode(search_string='_BEV\\[', replace_string='_EV[')(df=out_5460_1)
    # aviation to planes
    out_5460_1 = ColumnRenameRegexNode(search_string='aviation', replace_string='planes')(df=out_5460_1)
    # rail to trains
    out_5460_1 = ColumnRenameRegexNode(search_string='rail', replace_string='trains')(df=out_5460_1)
    # remove _same
    out_5460_1 = ColumnRenameRegexNode(search_string='_same\\[', replace_string='[')(df=out_5460_1)
    # ships = marine + IWW
    ...
    tra_ships_number = ColumnFilterNode(columns_to_drop=['tra_IWW[number]', 'tra_marine[number]'])(df=tra_ships_number)

    # Add dummy columns (not modelized in XCalc)

    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4567_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_2W_PHEV[number]', column_type='DOUBLE')(df=tra_ships_number)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4568_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_2W_FCEV[number]', column_type='DOUBLE')(df=out_4567_1)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4569_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_HDVM_ICE[number]', column_type='DOUBLE')(df=out_4568_1)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4570_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_HDVM_EV[number]', column_type='DOUBLE')(df=out_4569_1)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4571_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_HDVM_PHEV[number]', column_type='DOUBLE')(df=out_4570_1)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4572_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_HDVM_FCEV[number]', column_type='DOUBLE')(df=out_4571_1)
    # Add dummy 0 values for columns not modelled in XCalc and asked by Minerals
    out_4573_1 = ConstantValueColumnNode(column_value='0', new_column_name='tra_subways[number]', column_type='DOUBLE')(df=out_4572_1)
    # Join data for minerals
    out_6021_1 = JoinerNode(joiner='inner', left_input=['Country', 'Years'], right_input=['Country', 'Years'])(df_left=out_6004_1, df_right=out_4573_1)
    # new-veh-fleet[number] = veh-fleet-need[number] + veh-fleet-renewal[number]
    new_veh_fleet_number = MCDNode(operation_selection='x + y', output_name='new-veh-fleet[number]')(input_table_1=veh_fleet_need_number, input_table_2=veh_fleet_renewal_number)
    ...
    new_veh_fleet_number_2 = UseVariableNode(selected_variable='new-veh-fleet[number]')(input_table=new_veh_fleet_number_2)
    # final-new-veh-fleet[number] = new-veh-fleet[number] x technology-share[%]
    final_new_veh_fleet_number = MCDNode(operation_selection='x * y', output_name='final-new-veh-fleet[number]')(input_table_1=new_veh_fleet_number_2, input_table_2=technology_share_percent)
    # Group by  transport-user, vehicule-type, energy-carrier, motor-type (sum)
    final_new_veh_fleet_number = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=final_new_veh_fleet_number)
    # if new-veh-fleet[number] > 0 then set to 0
    ...
    tot_veh_fleet_remaining_number = MCDNode(operation_selection='x + y', output_name='tot-veh-fleet-remaining[number]')(input_table_1=new_veh_fleet_number, input_table_2=veh_fleet_remaining_number)
    # tot-veh-fleet-remaining [number]
    tot_veh_fleet_remaining_number = UseVariableNode(selected_variable='tot-veh-fleet-remaining[number]')(input_table=tot_veh_fleet_remaining_number)
    # final-veh-fleet-remaining[number] = tot-veh-fleet-remaining[number] x technology-share-fleet[%]
    final_veh_fleet_remaining_number = MCDNode(operation_selection='x * y', output_name='final-veh-fleet-remaining[number]')(input_table_1=tot_veh_fleet_remaining_number, input_table_2=technology_share_fleet_percent)
    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    final_veh_fleet_remaining_number = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=final_veh_fleet_remaining_number)
    ...
    out_5319_1 = JoinerNode(joiner='left', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=out_5314_1, df_right=technology_share_fleet_percent)
    # set to 0 if missing (no vehicule)
    out_5319_1 = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=out_5319_1)
    ...
    out_5315_1_2 = ColumnFilterNode(columns_to_drop=['final-veh-fleet-remaining[number]', 'final-veh-fleet-remaining-agg[number]', 'technology-share-fleet[%]', 'Timestep'])(df=out_5315_1)
    # OTS / FTS retrofit-switch [%]
    retrofit_switch_percent = ImportDataNode(trigram='tra', variable_name='retrofit-switch')()
    # only  final-veh-remaining-fleet done because X-switch node bugs if too many  columns
    out_5315_1 = ColumnFilterNode(columns_to_drop=['final-new-veh-fleet[number]', 'total-veh-fleet[number]', 'total-veh-fleet-agg[number]', 'final-veh-fleet-remaining-agg[number]', 'technology-share-fleet[%]', 'Timestep', 'technology-share[%]'])(df=out_5315_1)
    # Retrofit Switch thermic to electric
    out_9471_1 = XSwitchNode(col_energy='final-veh-fleet-remaining[number]', col_energy_carrier='motor-type', category_from_selected='thermic', category_to_selected='electric')(demand_table=out_5315_1, switch_table=retrofit_switch_percent, correlation_table=ratio_2)
    ...
    final_veh_fleet_remaining_number = UseVariableNode(selected_variable='final-veh-fleet-remaining[number]')(input_table=out_9471_1_excluded)
    ...
    # Group by  vehicule-type, motor-type (sum)
    out_9478_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'motor-type'], aggregation_method='Sum')(df=out_9478_1)
    out_1 = pd.concat([out_5410_1_2, out_9478_1.set_index(out_9478_1.index.astype(str) + '_dup')])
    # Group by  vehicule-type, motor-type (sum)
    out_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'motor-type'], aggregation_method='Sum')(df=out_1)
    ...
    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    out_9471_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=out_9471_1)
    # Remerge
    out_9492_1 = JoinerNode(joiner='left', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=out_5315_1_2, df_right=out_9471_1)
    ...
    # Fill with 0
    technology_share_percent = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=technology_share_percent)
    ...
    # technology-share [%]
    technology_share_percent_2 = ExportVariableNode(selected_variable='technology-share[%]')(input_table=out_1_2)
    ...
    # technology-share [%]
    technology_share_percent_2 = UseVariableNode(selected_variable='technology-share[%]')(input_table=technology_share_percent_2)
    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    technology_share_percent_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=technology_share_percent_2)
    ...
    share_percent = ColumnFilterNode(pattern='^.*$')(df=share_percent)
    # final-transport-demand[vkm] = veh-transport-demand[vkm] * technology-share[%]
    final_transport_demand_vkm = MCDNode(operation_selection='x * y', output_name='final-transport-demand[vkm]', fill_value_bool='Inner Join')(input_table_1=technology_share_percent_2, input_table_2=veh_transport_demand_vkm)
    ...
    final_transport_demand_tkm = MCDNode(operation_selection='x * y', output_name='final-transport-demand[tkm]')(input_table_1=technology_share_percent_2, input_table_2=transport_demand_tkm)
    # final-transport-demand [tkm]
    final_transport_demand_tkm_2 = ExportVariableNode(selected_variable='final-transport-demand[tkm]')(input_table=final_transport_demand_tkm)

    # HDV share in freight

    # final-transport-demand [tkm]
    final_transport_demand_tkm = UseVariableNode(selected_variable='final-transport-demand[tkm]')(input_table=final_transport_demand_tkm_2)
    # Group by year
    final_transport_demand_tkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=final_transport_demand_tkm)
    # Top: HDV
    final_transport_demand_tkm_4 = final_transport_demand_tkm.loc[final_transport_demand_tkm['vehicule-type'].isin(['HDV'])].copy()
    # Group by year
    final_transport_demand_tkm_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=final_transport_demand_tkm_4)
    # modal-share-HDV[%tkm] = final-transport-demand (HDV) [tkm] / final-transport-demand (all) [tkm]
    modal_share_HDV_percent_tkm = MCDNode(operation_selection='x / y', output_name='modal-share-HDV[%tkm]')(input_table_1=final_transport_demand_tkm_4, input_table_2=final_transport_demand_tkm_3)
    # modal-share-HDV[%tkm]
    modal_share_HDV_percent_tkm = ExportVariableNode(selected_variable='modal-share-HDV[%tkm]')(input_table=modal_share_HDV_percent_tkm)
    # transport-demand-per-cap [tkm/cap] = transport_demand [tkm] / Capita [cap]
    transport_demand_per_cap_tkm_per_cap = MCDNode(operation_selection='x / y', output_name='transport-demand-per-cap[tkm/cap]')(input_table_1=final_transport_demand_tkm_3, input_table_2=population_cap)
    ...
    final_transport_demand_pkm = MCDNode(operation_selection='x * y', output_name='final-transport-demand[pkm]')(input_table_1=technology_share_percent_2, input_table_2=transport_demand_pkm_3)

    # Final transport demand

    # final-transport-demand [pkm]
    final_transport_demand_pkm = ExportVariableNode(selected_variable='final-transport-demand[pkm]')(input_table=final_transport_demand_pkm)
    # Final transport demand (pkm)
    final_transport_demand_pkm_2 = UseVariableNode(selected_variable='final-transport-demand[pkm]')(input_table=final_transport_demand_pkm)
    # Group by  transport-user, vehicule-type, motor-type (sum)
    final_transport_demand_pkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type'], aggregation_method='Sum')(df=final_transport_demand_pkm_2)
    # transport-demand-per-cap [pkm/cap] = transport_demand [pkm] / Capita [cap]
    transport_demand_per_cap_pkm_per_cap = MCDNode(operation_selection='y / x', output_name='transport-demand-per-cap[pkm/cap]')(input_table_1=population_cap, input_table_2=final_transport_demand_pkm_3)
    # transport-demand-per-cap [pkm/cap]
    transport_demand_per_cap_pkm_per_cap = ExportVariableNode(selected_variable='transport-demand-per-cap[pkm/cap]')(input_table=transport_demand_per_cap_pkm_per_cap)
    ...
    out_5134_1 = JoinerNode(joiner='inner', left_input=['vehicule-type'], right_input=['vehicule-type'])(df_left=final_transport_demand_km, df_right=out_5133_1)
    ...
    out_5178_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'material'], aggregation_method='Sum')(df=out_5178_1)
    # Divide by baseyear
    out_7159_1 = DivideYear(in_var='final-transport-demand[km]', output_name='increase-rate[%]', reference_year=Globals.get().base_year)(df=out_5178_1)
    # Set 0 if divide by 0
    out_7159_1 = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=out_7159_1)
    # increase-rate [%]
    increase_rate_percent = ExportVariableNode(selected_variable='increase-rate[%]')(input_table=out_7159_1)
    # Keep rail only
    ...
    final_transport_demand_pkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=final_transport_demand_pkm_2)
    # final-transport-demand to total-rail-demand
    final_transport_demand_pkm_3 = ColumnRenameRegexNode(search_string='(.*demand.*)', replace_string='total-rail-demand[pkm]')(df=final_transport_demand_pkm_3)
    # Keep CEV only
    ...
    corr_factor_trolley_percent = MCDNode(operation_selection='x / y', output_name='corr-factor-trolley[%]')(input_table_1=final_transport_demand_pkm_2, input_table_2=final_transport_demand_pkm_3)

    # Add trolley-cables category
    # = copy values from rails

    # Set 0 if divide by 0
    corr_factor_trolley_percent = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=corr_factor_trolley_percent)
    # corr-factor-trolley [%]
    corr_factor_trolley_percent = ExportVariableNode(selected_variable='corr-factor-trolley[%]')(input_table=corr_factor_trolley_percent)
    # Group by  Country, Years (mean)
    corr_factor_trolley_percent = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=corr_factor_trolley_percent)
    ...
    length_km = ImportDataNode(trigram='tra', variable_name='length', variable_type='OTS (only)')()
    # Same as last available year
    length_km = AddMissingYearsNode()(df_data=length_km)
    ...
    corr_length_km = MCDNode(operation_selection='x * y', output_name='corr-length[km]')(input_table_1=out_5179_1, input_table_2=out_5173_1)
    # corr-length[km] = (replace) corr-length[km] x corr-factor-trolley[%]  If missing corr-factor-torlley set to 1
    corr_length_km = MCDNode(operation_selection='x * y', output_name='corr-length[km]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=corr_length_km, input_table_2=out_5954_1)
    # corr-length [km]
    corr_length_km = ExportVariableNode(selected_variable='corr-length[km]')(input_table=corr_length_km)
    # material-length-demand[km] = increase-rate[%] * corr-length[km]
    material_length_demand_km = MCDNode(operation_selection='x * y', output_name='material-length-demand[km]')(input_table_1=increase_rate_percent, input_table_2=corr_length_km)

    # Material length demand

    # material-length-demand [km]
    material_length_demand_km = ExportVariableNode(selected_variable='material-length-demand[km]')(input_table=material_length_demand_km)
    # OTS / FTS fuel-switch [%]
    fuel_switch_percent = ImportDataNode(trigram='tra', variable_name='fuel-switch')()
    # Fuel Switch fffuels to biofuels
    out_9542_1 = XSwitchNode(col_energy='veh-efficiency-km-avg[MJ/km]')(demand_table=veh_efficiency_km_avg_MJ_per_km, switch_table=fuel_switch_percent, correlation_table=ratio)
    # Fuel Switch fffuels to efuels
    out_9543_1 = XSwitchNode(col_energy='veh-efficiency-km-avg[MJ/km]', category_to_selected='synfuels')(demand_table=out_9542_1, switch_table=fuel_switch_percent, correlation_table=ratio)
    ...
    tra_combustion_emission_factor_Mt_per_TWh = ImportDataNode(trigram='tra', variable_name='tra-combustion-emission-factor', variable_type='RCP')()
    # emissions-veh[Mt/km][Mt/km] = veh-efficiency-km-avg[TWh/km] * tra-combustion-emission-factor [Mt/TWh]
    emissions_veh_Mt_per_km = MCDNode(operation_selection='x * y', output_name='emissions-veh[Mt/km]')(input_table_1=veh_efficiency_km_avg_TWh_per_km, input_table_2=tra_combustion_emission_factor_Mt_per_TWh)
    ...
    # Group by year
    emissions_veh_g_per_km = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'fleet-age', 'gaes'], aggregation_method='Sum')(df=emissions_veh_g_per_km)
    # Top: only new
    ...
    # emissions-veh[g/km]
    emissions_veh_g_per_km = ExportVariableNode(selected_variable='emissions-veh[g/km]')(input_table=emissions_veh_g_per_km)
    ...
    material_length_demand_km = UseVariableNode(selected_variable='material-length-demand[km]')(input_table=material_length_demand_km)
    out_7164_1 = pd.concat([out_1, material_length_demand_km.set_index(material_length_demand_km.index.astype(str) + '_dup')])
    # All for : Industry
    out_7164_1 = ColumnFilterNode(pattern='^.*$')(df=out_7164_1)

    # For : Minerals

    # Pivot

    out_5401_1, _, _ = PivotingNode(agg_dict={'material-length-demand[km]': 'sum'}, column_name_option='Pivot name+Aggregation name', column_name_policy='Keep original name(s)', list_group_columns=['Years', 'Country'], list_pivots=['material'])(df=material_length_demand_km)
    out_5402_1 = MissingValueColumnFilterNode(missing_threshold=0.9, type_of_pattern='Manual')(df=out_5401_1)
    # Same as used in Industry
    out_5402_1 = ColumnRenameRegexNode(search_string='(.*)\\+.*(\\[.*)', replace_string='tra_length_$1$2')(df=out_5402_1)
    # remove s from rails
    out_5402_1 = ColumnRenameRegexNode(search_string='rails', replace_string='rail')(df=out_5402_1)
    # remove s from roads
    out_5402_1 = ColumnRenameRegexNode(search_string='roads', replace_string='road')(df=out_5402_1)

    # Rename

    # Keep all
    out_5402_1 = ColumnFilterNode(pattern='^.*$')(df=out_5402_1)
    # Remove length
    out_5402_1 = ColumnRenameRegexNode(search_string='length_', replace_string='')(df=out_5402_1)
    # Join data for minerals
    out_5466_1 = JoinerNode(joiner='inner', left_input=['Country', 'Years'], right_input=['Country', 'Years'])(df_left=out_5402_1, df_right=out_6021_1)
    # All for : Minerals
    out_5466_1 = ColumnFilterNode(pattern='^.*$')(df=out_5466_1)
    # Years
    out_6036_1 = out_5466_1.assign(Years=out_5466_1['Years'].astype(str))
    # RCP costs-by-transport-infra [MEUR/km] From TECH
    costs_by_transport_infra_MEUR_per_km = ImportDataNode(trigram='tec', variable_name='costs-by-transport-infra', variable_type='RCP')()
    ...
    out_9430_1 = ComputeCosts(activity_variable='material-length-demand[km]')(df_activity=material_length_demand_km, df_unit_costs=costs_by_transport_infra_MEUR_per_km, df_price_indices=price_indices_, df_wacc=wacc_percent, module_name=module_name)
    # RCP transport-infra-cost-user [-]
    transport_infra_cost_user_ = ImportDataNode(trigram='tra', variable_name='transport-infra-cost-user', variable_type='RCP')()
    # capex[MEUR] = capex[MEUR] * vehicle-cost-user[-]
    capex_MEUR_2 = MCDNode(operation_selection='x * y', output_name='capex[MEUR]')(input_table_1=out_9430_1, input_table_2=transport_infra_cost_user_)
    # Group by  Country, Years, material (sum)
    capex_MEUR = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'material'], aggregation_method='Sum')(df=capex_MEUR_2)
    ...
    out_9401_1 = ComputeCosts(activity_variable='final-veh-fleet-ind[number]')(df_activity=final_veh_fleet_ind_number_2, df_unit_costs=costs_by_vehicle_MEUR_per_number, df_price_indices=price_indices_, df_wacc=wacc_percent, module_name=module_name)
    # RCP vehicle-cost-user [-]
    vehicle_cost_user_ = ImportDataNode(trigram='tra', variable_name='vehicle-cost-user', variable_type='RCP')()
    # capex[MEUR] = capex[MEUR] * vehicle-cost-user[-]
    capex_MEUR = MCDNode(operation_selection='x * y', output_name='capex[MEUR]')(input_table_1=out_9401_1, input_table_2=vehicle_cost_user_)
    ...
    capex_MEUR_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'energy-carrier', 'motor-type'], aggregation_method='Sum')(df=capex_MEUR_excluded)
    ...
    capex_MEUR = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type'], aggregation_method='Sum')(df=capex_MEUR)
    ...
    final_veh_fleet_remaining_number = ExportVariableNode(selected_variable='final-veh-fleet-remaining[number]')(input_table=technology_share_percent)
    ...
    # final-veh-fleet-remaining [number]
    final_veh_fleet_remaining_number = ExportVariableNode(selected_variable='final-veh-fleet-remaining[number]')(input_table=out_1_2)
    # total-veh-fleet [number]
    total_veh_fleet_number = ExportVariableNode(selected_variable='total-veh-fleet[number]')(input_table=technology_share_percent)
    ...
    final_new_veh_fleet_number = ExportVariableNode(selected_variable='final-new-veh-fleet[number]')(input_table=technology_share_percent)
    ...
    # total-veh-fleet[number] = final-new-veh-fleet[number] + final-veh-fleet-remaining[number]
    total_veh_fleet_number = MCDNode(operation_selection='x + y', output_name='total-veh-fleet[number]')(input_table_1=final_new_veh_fleet_number, input_table_2=out_5583_1)
    out_5576_1 = pd.concat([out_5560_1, total_veh_fleet_number.set_index(total_veh_fleet_number.index.astype(str) + '_dup')])
    # total-veh-fleet [number]
    total_veh_fleet_number = ExportVariableNode(selected_variable='total-veh-fleet[number]')(input_table=out_5576_1)

    # Number of passenger LDV

    # total-veh-fleet [number]
    total_veh_fleet_number_2 = UseVariableNode(selected_variable='total-veh-fleet[number]')(input_table=total_veh_fleet_number)
    # Top: Passenger
    ...
    # Group by  year
    total_veh_fleet_number_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=total_veh_fleet_number_3)
    # total-veh-fleet[number] to  total-veh-fleet-passenger-LDV[number]
    total_veh_fleet_number_3 = ColumnRenameRegexNode(search_string='total-veh-fleet', replace_string='total-veh-fleet-passenger-LDV')(df=total_veh_fleet_number_3)
    # total-veh-fleet-passenger-LDV[number]
    total_veh_fleet_passenger_LDV_number = ExportVariableNode(selected_variable='total-veh-fleet-passenger-LDV[number]')(input_table=total_veh_fleet_number_3)

    # For : Pathway Explorer

    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    total_veh_fleet_number_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=total_veh_fleet_number_2)
    # Split motor-type
    total_veh_fleet_number_excluded = total_veh_fleet_number_3.loc[total_veh_fleet_number_3['motor-type'].isin(['PHEV', 'PHEVCE'])].copy()
    total_veh_fleet_number_3 = total_veh_fleet_number_3.loc[~total_veh_fleet_number_3['motor-type'].isin(['PHEV', 'PHEVCE'])].copy()
    total_veh_fleet_number_excluded = total_veh_fleet_number_excluded.loc[~total_veh_fleet_number_excluded['energy-carrier'].isin(['electricity'])].copy()
    ...
    out_9422_1 = ComputeCosts(activity_variable='total-veh-fleet[number]', cost_type='OPEX')(df_activity=total_veh_fleet_number_2, df_unit_costs=costs_by_vehicle_MEUR_per_number, df_price_indices=price_indices_, df_wacc=wacc_percent, module_name=module_name)
    # opex[MEUR] = opex[MEUR] * vehicle-cost-user[-]
    opex_MEUR = MCDNode(operation_selection='x * y', output_name='opex[MEUR]')(input_table_1=out_9422_1, input_table_2=vehicle_cost_user_)
    out_5577_1 = pd.concat([out_5574_1, final_new_veh_fleet_number.set_index(final_new_veh_fleet_number.index.astype(str) + '_dup')])
    # final-new-veh-fleet [number]
    final_new_veh_fleet_number = ExportVariableNode(selected_variable='final-new-veh-fleet[number]')(input_table=out_5577_1)
    out_5330_1 = JoinerNode(joiner='inner', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=total_veh_fleet_number, df_right=final_new_veh_fleet_number)
    out_5331_1 = JoinerNode(joiner='inner', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=out_5330_1, df_right=final_veh_fleet_remaining_number)
    # Left Join
    out_5367_1 = JoinerNode(joiner='left', left_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], right_input=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'])(df_left=out_5331_1, df_right=out_5365_1)
    ...
    veh_efficiency_pkm_MJ_per_pkm = ExportVariableNode(selected_variable='veh-efficiency-pkm[MJ/pkm]')(input_table=out_5350_1)
    ...
    veh_efficiency_pkm_MJ_per_pkm = UseVariableNode(selected_variable='veh-efficiency-pkm[MJ/pkm]')(input_table=out_5369_1)
    # energy-demand[MJ] = final-transport-demand[pkm] * vehicle-efficiency[MJ/pkm]
    energy_demand_MJ = MCDNode(operation_selection='x * y', output_name='energy-demand[MJ]')(input_table_1=final_transport_demand_pkm, input_table_2=veh_efficiency_pkm_MJ_per_pkm)
    # veh-efficiency-tkm [MJ/tkm]
    veh_efficiency_tkm_MJ_per_tkm = ExportVariableNode(selected_variable='veh-efficiency-tkm[MJ/tkm]')(input_table=out_5350_1)
    ...
    veh_efficiency_tkm_MJ_per_tkm = UseVariableNode(selected_variable='veh-efficiency-tkm[MJ/tkm]')(input_table=out_5368_1)
    # energy-demand[MJ] = final-transport-demand[tkm] * vehicle-efficiency[MJ/tkm]
    energy_demand_MJ_3 = MCDNode(operation_selection='x * y', output_name='energy-demand[MJ]')(input_table_1=final_transport_demand_tkm_2, input_table_2=veh_efficiency_tkm_MJ_per_tkm)
    # veh-efficiency-km [MJ/km]
    veh_efficiency_km_MJ_per_km = ExportVariableNode(selected_variable='veh-efficiency-km[MJ/km]')(input_table=out_5350_1)
    ...
    veh_efficiency_km_MJ_per_km = UseVariableNode(selected_variable='veh-efficiency-km[MJ/km]')(input_table=out_5274_1)
    # Pins (all selected)
    veh_transport_demand_vkm = ColumnFilterNode(pattern='^.*$')(df=veh_transport_demand_vkm_2)
    # veh-transport-demand [vkm] from tkm
    veh_transport_demand_vkm = UseVariableNode(selected_variable='veh-transport-demand[vkm]')(input_table=veh_transport_demand_vkm)
    # transport-demand[vkm] = veh-transport-demand[vkm] * technology-share[%]
    final_transport_demand_vkm = MCDNode(operation_selection='x * y', output_name='final-transport-demand[vkm]')(input_table_1=technology_share_percent_2, input_table_2=veh_transport_demand_vkm)
    ...
    final_transport_demand_vkm_2 = ExportVariableNode(selected_variable='final-transport-demand[vkm]')(input_table=out_1_2)
    # final-transport-demand[vkm] 
    final_transport_demand_vkm = UseVariableNode(selected_variable='final-transport-demand[vkm]')(input_table=final_transport_demand_vkm_2)
    ...
    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    final_transport_demand_vkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=final_transport_demand_vkm_3)
    # final-transport-demand[vkm] to  final-transport-demand-total[vkm]
    final_transport_demand_vkm_3 = ColumnRenameRegexNode(search_string='final-transport-demand', replace_string='final-transport-demand-total')(df=final_transport_demand_vkm_3)
    # final-transport-demand-total[vkm]
    final_transport_demand_total_vkm = ExportVariableNode(selected_variable='final-transport-demand-total[vkm]')(input_table=final_transport_demand_vkm_3)
    # KPI
    ...
    # Group by year
    final_transport_demand_vkm_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type'], aggregation_method='Sum')(df=final_transport_demand_vkm_4)
    ...
    # Group by year
    final_transport_demand_vkm_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type'], aggregation_method='Sum')(df=final_transport_demand_vkm_4)
    # elec-share[%vkm] = final-transport-demand  (HDV, energy-carrier=elec) [vkm] / final-transport-demand  (HDV, energy-carrier=all) [vkm]
    elec_share_percent_vkm = MCDNode(operation_selection='x / y', output_name='elec-share[%vkm]')(input_table_1=final_transport_demand_vkm_4, input_table_2=final_transport_demand_vkm_3)
    # elec-share[%tkm]
    elec_share_percent_vkm = ExportVariableNode(selected_variable='elec-share[%vkm]')(input_table=elec_share_percent_vkm)
    # Top: LDV
    ...
    elec_share_percent_vkm_2 = MCDNode(operation_selection='x / y', output_name='elec-share[%vkm]')(input_table_1=final_transport_demand_vkm_4, input_table_2=final_transport_demand_vkm_3)
    # elec-share[%vkm]
    elec_share_percent_vkm_2 = ExportVariableNode(selected_variable='elec-share[%vkm]')(input_table=elec_share_percent_vkm_2)
    # energy-demand[MJ] = final-transport-demand[vkm] * vehicle-efficiency[MJ/km]
    energy_demand_MJ_2 = MCDNode(operation_selection='x * y', output_name='energy-demand[MJ]')(input_table_1=final_transport_demand_vkm_2, input_table_2=veh_efficiency_km_MJ_per_km)
    ...
    # Group by  transport-user, vehicule-type, motor-type, energy-carrier (sum)
    energy_demand_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'vehicule-type', 'motor-type', 'energy-carrier'], aggregation_method='Sum')(df=energy_demand_TWh)
    ...
    # OTS (only) fuel-mix [%]
    fuel_mix_percent = ImportDataNode(trigram='tra', variable_name='fuel-mix', variable_type='OTS (only)')()
    # Same as last available year
    fuel_mix_percent = AddMissingYearsNode()(df_data=fuel_mix_percent)
    ...
    out_9418_1 = ComputeCosts(cost_type='OPEX')(df_activity=final_transport_demand_vkm_2, df_unit_costs=costs_by_vehicle_MEUR_per_number, df_price_indices=price_indices_, df_wacc=wacc_percent, module_name=module_name)
    # opex[MEUR] = opex[MEUR] * vehicle-cost-user[-]
    opex_MEUR_2 = MCDNode(operation_selection='x * y', output_name='opex[MEUR]')(input_table_1=out_9418_1, input_table_2=vehicle_cost_user_)
    opex_MEUR_3 = pd.concat([opex_MEUR, opex_MEUR_2.set_index(opex_MEUR_2.index.astype(str) + '_dup')])
    # Group by  Country, Years, cost-user (sum)
    opex_MEUR_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'cost-user'], aggregation_method='Sum')(df=opex_MEUR_3)
    ...
    # Group by  Country, Years, vehicle-type, energy-carrier, motor-type (sum)
    opex_MEUR = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'energy-carrier', 'motor-type'], aggregation_method='Sum')(df=opex_MEUR)
    ...
    # Set 0
    out_1 = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory']], FixedValue=None)(df=out_1)
    out_9565_1 = pd.concat([out_1, MEUR.set_index(MEUR.index.astype(str) + '_dup')])

    # Hard codé ici ==> à terme, introduire dans une google sheet !

    # Correlation for fuel mix
    out_4646_1 = TableCreatorNode(df=pd.DataFrame(columns=['category_fffuels', 'category_biofuel', 'category_efuel'], data=[['liquid-ff-diesel', 'liquid-bio-diesel', 'liquid-syn-diesel'], ['liquid-ff-gasoline', 'liquid-bio-gasoline', 'liquid-syn-gasoline'], ['gaseous-ff-natural', 'gaseous-bio', 'gaseous-syn'], ['liquid-ff-kerosene', 'liquid-bio-kerosene', 'liquid-syn-kerosene'], ['liquid-ff-marinefueloil', 'liquid-bio-marinefueloil', 'liquid-syn-marinefueloil']]))()
    # Fuel Mix fffuels to biofuels
    out_5061_1 = FuelMixNode(col_category_to='category_biofuel')(input_energy=energy_demand_TWh, switch_values=fuel_mix_percent, values_to_from=out_4646_1)
    # Fuel Mix fffuels to efuels
    out_5067_1 = FuelMixNode(col_category_to='category_efuel')(input_energy=out_5061_1, switch_values=fuel_mix_percent, values_to_from=out_4646_1)
    ...
    energy_demand_TWh = ExportVariableNode(selected_variable='energy-demand[TWh]')(input_table=out_5067_1)
    ...
    energy_demand_TWh_3 = ImportDataNode(trigram='tra', variable_name='energy-demand', variable_type='Calibration')()
    # Apply Calibration on energy-demand[TWh]
    _, out_7107_2, out_7107_3 = CalibrationNode(data_to_be_cal='energy-demand[TWh]', data_cal='energy-demand[TWh]')(input_table=energy_demand_TWh_2, cal_table=energy_demand_TWh_3)
    # Apply cal_rate to energy-demand[TWh]
    energy_demand_TWh = MCDNode(operation_selection='x * y', output_name='energy-demand[TWh]')(input_table_1=energy_demand_TWh, input_table_2=out_7107_2)
    ...
    # Fuel Switch fffuels to biofuels
    out_7160_1 = XSwitchNode()(demand_table=energy_demand_TWh, switch_table=fuel_switch_percent, correlation_table=ratio)
    # Fuel Switch fffuels to efuels
    out_7161_1 = XSwitchNode(category_to_selected='synfuels')(demand_table=out_7160_1, switch_table=fuel_switch_percent, correlation_table=ratio)

    # Split ETS / non-ETS

    # RCP transport-ets-share [%] FROM TECH
    transport_ets_share_percent = ImportDataNode(trigram='tec', variable_name='transport-ets-share', variable_type='RCP')()
    ...
    energy_demand_TWh = MCDNode(operation_selection='x * y', output_name='energy-demand[TWh]', fill_value_bool='Left [x] Outer Join', fill_value=1.0)(input_table_1=out_7161_1, input_table_2=transport_ets_share_)
    ...
    aviation_domestic_share_percent = ImportDataNode(trigram='tra', variable_name='aviation-domestic-share', variable_type='OTS (only)')()
    # Same as last available year
    aviation_domestic_share_percent = AddMissingYearsNode()(df_data=aviation_domestic_share_percent)
    ...
    energy_demand_TWh = MCDNode(operation_selection='x * (1-y)', output_name='energy-demand[TWh]')(input_table_1=out_9203_1_excluded, input_table_2=aviation_domestic_share_percent)
    ...
    energy_demand_TWh_2 = MCDNode(operation_selection='x * y', output_name='energy-demand[TWh]')(input_table_1=out_9203_1_excluded, input_table_2=aviation_domestic_share_percent)
    ...
    # energy-demand [TWh]
    energy_demand_TWh = ExportVariableNode(selected_variable='energy-demand[TWh]')(input_table=out_9452_1)
    ...
    emissions_Mt = MCDNode(operation_selection='x * y', output_name='emissions[Mt]')(input_table_1=energy_demand_TWh, input_table_2=tra_combustion_emission_factor_Mt_per_TWh)
    # Group by  gaes, domestic-type (sum)
    emissions_Mt_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'gaes', 'domestic-type'], aggregation_method='Sum')(df=emissions_Mt)
    ...
    emissions_Mt_3 = ImportDataNode(trigram='tra', variable_name='emissions', variable_type='Calibration')()
    # Apply Calibration on emissions[Mt]
    _, out_7103_2, out_7103_3 = CalibrationNode(data_to_be_cal='emissions[Mt]', data_cal='emissions[Mt]')(input_table=emissions_Mt_2, cal_table=emissions_Mt_3)
    # Apply cal_rate emissions[Mt] = emissions[Mt] * cal_rate
    emissions_Mt = MCDNode(operation_selection='x * y', output_name='emissions[Mt]')(input_table_1=emissions_Mt, input_table_2=out_7103_2)
    ...
    cal_rate_emissions_Mt = UseVariableNode(selected_variable='cal_rate_emissions[Mt]')(input_table=out_7103_3)
    ...
    energy_demand_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'energy-carrier', 'transport-user', 'motor-type', 'domestic-type'], aggregation_method='Sum')(df=energy_demand_TWh_2)

    # For : Air Quality

    # Group by country, year, veh-user, motor-type, energy-carrier,  domestic-type
    energy_demand_TWh_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'energy-carrier', 'transport-user', 'motor-type', 'domestic-type'], aggregation_method='Sum')(df=energy_demand_TWh)
    ...
    # All for : Air Quality
    demand = ColumnFilterNode(pattern='^.*$')(df=demand)
    # GroupBy transport-user
    energy_demand_TWh_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'vehicule-type', 'energy-carrier', 'transport-user'], aggregation_method='Sum')(df=energy_demand_TWh)
    # Top: only passenger
    ...
    energy_demand_urban_TWh = MCDNode(operation_selection='x * y', output_name='energy-demand-urban[TWh]')(input_table_1=urban_share_percent_vkm, input_table_2=energy_demand_TWh_3)
    # GroupBy transport-user
    energy_demand_urban_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'energy-carrier'], aggregation_method='Sum')(df=energy_demand_urban_TWh)
    # energy-demand-urban[TWh]
    energy_demand_urban_TWh = ExportVariableNode(selected_variable='energy-demand-urban[TWh]')(input_table=energy_demand_urban_TWh)
    # GroupBy transport-user
    energy_demand_urban_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user'], aggregation_method='Sum')(df=energy_demand_urban_TWh)
    # share-energy-demand-urban[%TWh] = energy-demand-urban[TWh] (per vector) / energy-demand-urban[TWh] (total)
    share_energy_demand_urban_percent_TWh = MCDNode(operation_selection='x / y', output_name='share-energy-demand-urban[%TWh]')(input_table_1=energy_demand_urban_TWh, input_table_2=energy_demand_urban_TWh_2)
    # share-energy-demand-urban[%TWh]
    share_energy_demand_urban_percent_TWh = ExportVariableNode(selected_variable='share-energy-demand-urban[%TWh]')(input_table=share_energy_demand_urban_percent_TWh)
    # KPI
    ...
    # energy-demand [TWh]
    energy_demand_TWh_3 = UseVariableNode(selected_variable='energy-demand[TWh]')(input_table=energy_demand_TWh)

    # For : Pathway Explorer

    # energy-demand[TWh]
    energy_demand_TWh_5 = UseVariableNode(selected_variable='energy-demand[TWh]')(input_table=energy_demand_TWh_3)
    ...
    energy_demand_TWh_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'transport-user', 'domestic-type'], aggregation_method='Sum')(df=energy_demand_TWh_4)
    ...
    energy_demand_TWh_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier', 'domestic-type'], aggregation_method='Sum')(df=energy_demand_TWh_4)
    ...
    energy_demand_TWh_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier', 'ets-or-not'], aggregation_method='Sum')(df=energy_demand_TWh_4)
    ...
    energy_demand_TWh_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'ets-or-not'], aggregation_method='Sum')(df=energy_demand_TWh_4)
    ...
    energy_demand_TWh_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'domestic-type'], aggregation_method='Sum')(df=energy_demand_TWh_excluded)
    ...
    # ALL RESULTS
    out_9213_1 = ColumnFilterNode(pattern='^.*$')(df=out_9213_1)
    # Years
    ...
    energy_demand_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=energy_demand_TWh_3)
    # All for : Power Oil Refineries
    energy_demand_TWh_2 = ColumnFilterNode(pattern='^.*$')(df=energy_demand_TWh_2)
    # All for : Employment
    energy_demand_TWh_3 = ColumnFilterNode(pattern='^.*$')(df=energy_demand_TWh_2)
    # Employment = dummy !
    out_1736_1 = ConstantValueColumnNode(column_value='', new_column_name='dummy', column_type='STRING')(df=energy_demand_TWh_3)
    # Top: fossil-fuel
    ...
    # Group by year
    energy_demand_TWh_excluded_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_demand_TWh_excluded_excluded_excluded)
    # energy-demand[TWh] to energy-demand-elec[TWh]
    energy_demand_TWh_excluded_excluded_excluded = ColumnRenameRegexNode(search_string='energy-demand', replace_string='energy-demand-elec')(df=energy_demand_TWh_excluded_excluded_excluded)
    # Group by year
    energy_demand_TWh_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_demand_TWh_excluded_excluded_2)
    # energy-demand[TWh] to energy-demand-biofuel[TWh]
    energy_demand_TWh_excluded_excluded = ColumnRenameRegexNode(search_string='energy-demand', replace_string='energy-demand-biofuel')(df=energy_demand_TWh_excluded_excluded)
    # KPI
    ...
    # Group by year
    energy_demand_TWh_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_demand_TWh_excluded_2)
    # energy-demand[TWh] to energy-demand-efuel-H2[TWh]
    energy_demand_TWh_excluded = ColumnRenameRegexNode(search_string='energy-demand', replace_string='energy-demand-efuel-hydrogene')(df=energy_demand_TWh_excluded)
    # KPI
    ...
    # Group by year
    energy_demand_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_demand_TWh_3)
    # energy-demand[TWh] to energy-demand-ff[TWh]
    energy_demand_TWh = ColumnRenameRegexNode(search_string='energy-demand', replace_string='energy-demand-ff')(df=energy_demand_TWh)
    # KPI
    ...
    out_9212_1 = AddTrigram()(module_name=module_name, df=out_1)
    # All for : Pathway Explorer
    out_9212_1 = ColumnFilterNode(pattern='^.*$')(df=out_9212_1)

    # Cal_rate for energy-demand[TWh]

    # Cal rate for  energy-demand[TWh]
    cal_rate_energy_demand_TWh = UseVariableNode(selected_variable='cal_rate_energy-demand[TWh]')(input_table=out_7107_3)
