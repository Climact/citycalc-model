...
def metanode_9000(port_01, port_02, port_03, port_04, port_05):
    ...
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh = UseVariableNode(selected_variable='elc_net-energy-production-by-carrier-primary-carrier-way-of-prod[TWh]')(input_table=port_05)
    # Group by  Country, Years, primary-energy-carrier (sum)
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'primary-energy-carrier'], aggregation_method='Sum')(df=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh)
    # Remove primary-energy-carrier with no values ("")
    out_8986_1 = RowFilterNode(filter_type='StringComp_RowFilter', that_column='primary-energy-carrier', include=True, pattern='..*', case_sensitive=True, is_reg_exp=True)(df=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_2)
    ...
    elc_energy_demand_by_energy_carrier_TWh = UseVariableNode(selected_variable='elc_energy-demand-by-energy-carrier[TWh]')(input_table=port_05)
    ...
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_2)
    ...
    # Group by Country, Years, energy-carrier (SUM)
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_excluded)
    # res-type = RES
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_excluded['res-type'] = "RES"
    # Group by Country, Years, energy-carrier (SUM)
    elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_3)
    # res-type = nonRES
    ...
    pct_RES_percent = MCDNode(operation_selection='y / x', output_name='pct-RES[%]')(input_table_1=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh, input_table_2=elc_net_energy_production_by_carrier_primary_carrier_way_of_prod_TWh_2)
    # sector = agr
    port_03['sector'] = "agr"
    # energy-demand [TWh]
    energy_demand_TWh = UseVariableNode(selected_variable='energy-demand[TWh]')(input_table=port_04)
    # sector = ind
    ...
    out_3931_1 = RowFilterNode(filter_type='StringComp_RowFilter', that_column='energy-carrier', include=False, pattern='heat|.*kerosene.*|.*marinefueloil.*|ambiant', case_sensitive=True, is_reg_exp=True)(df=out_1)
    # Keep : - marinefueoil (bio, syn, ff) - kersoene (bio, syn, ff)
    out_3946_1 = RowFilterNode(filter_type='StringComp_RowFilter', that_column='energy-carrier', include=True, pattern='.*kerosene.*|.*marinefueloil.*', case_sensitive=True, is_reg_exp=True)(df=out_1)
    # sector = tra-bunkers
    ...
    out_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier', 'sector'], aggregation_method='Sum')(df=out_1)
    ...
    out_3950_1_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'sector', 'res-type'], aggregation_method='Sum')(df=out_3950_1)
    ...
    out_3950_1_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'res-type'], aggregation_method='Sum')(df=out_3950_1)
    # Group by  Country, Years, (sum)
    out_3950_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=out_3950_1)
    # res_energy-demand[%] = energy-demand[TWh] by res-type / energy-demand[TWh] total
    res_energy_demand_percent = MCDNode(operation_selection='x / y', output_name='res_energy-demand[%]')(input_table_1=out_3950_1_2, input_table_2=out_3950_1)
