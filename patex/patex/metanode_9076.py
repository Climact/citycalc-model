...
def metanode_9076(port_01, port_02, port_03, port_04):
    ...
    out_9323_1 = TableCreatorNode(df=pd.DataFrame(columns=['energy-carrier', 'energy-carrier-agg'], data=[['liquid-bio', 'liquid-bio-agg'], ['liquid-bio-diesel', 'liquid-bio-agg'], ['liquid-bio-gasoline', 'liquid-bio-agg'], ['liquid-bio-kerosene', 'liquid-bio-agg'], ['liquid-bio-marinefueloil', 'liquid-bio-agg'], ['solid-biomass', 'solid-biomass'], ['gaseous-bio', 'gaseous-bio']]))()

    # Adapt data from other module => pivot and co

    # ELECTRICITY
    out_7991_1 = InjectVariablesDataNode()(df=port_04)
    ...
    # energy-demand [TWh]
    energy_demand_TWh = UseVariableNode(selected_variable='energy-demand[TWh]')(input_table=out_7991_1)
    ...
    # LAND-USE
    out_7989_1 = InjectVariablesDataNode()(df=port_03)
    ...
    potential_production_for_industry_t = UseVariableNode(selected_variable='potential-production-for-industry[t]')(input_table=out_7989_1)
    # energy-production [TWh]
    energy_production_TWh_3 = UseVariableNode(selected_variable='energy-production[TWh]')(input_table=out_7989_1)
    # OTS (only) energy-production [TWh] (additionnal production not modelised = sludges / agro-food-industrial wastes)
    energy_production_TWh = ImportDataNode(trigram='agr', variable_name='energy-production', variable_type='OTS (only)')()
    # Same as last available year
    energy_production_TWh_2 = AddMissingYearsNode()(df_data=energy_production_TWh)
    # wood-production [m3]
    wood_production_m3 = UseVariableNode(selected_variable='wood-production[m3]')(input_table=out_7989_1)
    # OTS (only) forest-harvest-for-bioenergy [%]
    forest_harvest_for_bioenergy_percent = ImportDataNode(trigram='agr', variable_name='forest-harvest-for-bioenergy', variable_type='OTS (only)')()
    # Same as last available year
    forest_harvest_for_bioenergy_percent = AddMissingYearsNode()(df_data=forest_harvest_for_bioenergy_percent)
    # potential-production-for-industry[m3] = wood-production[m3] * (1 - forest-harvest-for-bioenergy[%])
    potential_production_for_industry_m3 = MCDNode(operation_selection='x * (1-y)', output_name='potential-production-for-industry[m3]')(input_table_1=wood_production_m3, input_table_2=forest_harvest_for_bioenergy_percent)
    # RCP : forest-m3-to-t-conv-factor [t/m3]
    forest_m3_to_t_conv_factor_t_per_m3 = ImportDataNode(trigram='agr', variable_name='forest-m3-to-t-conv-factor', variable_type='RCP')()
    # potential-production-for-industry[t] = potential-energy-production[m3] * forest-m3-to-t-conv-factor [t/m3]
    potential_production_for_industry_t_2 = MCDNode(operation_selection='x * y', output_name='potential-production-for-industry[t]')(input_table_1=potential_production_for_industry_m3, input_table_2=forest_m3_to_t_conv_factor_t_per_m3)
    ...
    potential_production_for_industry_t = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'origin'], aggregation_method='Sum')(df=potential_production_for_industry_t)
    ...
    potential_energy_production_m3 = MCDNode(operation_selection='x * y', output_name='potential-energy-production[m3]')(input_table_1=wood_production_m3, input_table_2=forest_harvest_for_bioenergy_percent)
    # RCP : forest-m3-to-TWh-conv-factor [TWh/m3]
    forest_m3_to_TWh_conv_factor_TWh_per_m3 = ImportDataNode(trigram='agr', variable_name='forest-m3-to-TWh-conv-factor', variable_type='RCP')()
    # energy-production[TWh] = potential-energy-production[m3] * forest-m3-to-TWh-conv-factor [TWh/m3]
    energy_production_TWh = MCDNode(operation_selection='x * y', output_name='energy-production[TWh]')(input_table_1=potential_energy_production_m3, input_table_2=forest_m3_to_TWh_conv_factor_TWh_per_m3)
    # Group by Country, Years, energy-carrier, origin (sum)
    energy_production_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'origin', 'energy-carrier'], aggregation_method='Sum')(df=energy_production_TWh)
    # AGRICULTURE
    out_5965_1 = InjectVariablesDataNode()(df=port_02)
    # energy-production [TWh]
    energy_production_TWh_4 = UseVariableNode(selected_variable='energy-production[TWh]')(input_table=out_5965_1)
    ...
    energy_production_TWh_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_production_TWh)
    # energy-production [TWh]
    ...
    buffer_TWh = MCDNode(operation_selection='x - y', output_name='buffer[TWh]', fill_value_bool='Left [x] Outer Join')(input_table_1=energy_demand_TWh_2, input_table_2=energy_production_TWh_3)
    ...
    # Join of inputs
    out_693_1 = JoinerNode(joiner='outer', left_input=['energy-carrier'], right_input=['energy-carrier'])(df_left=energy_demand_TWh, df_right=out_9323_1)
    ...
    out_9325_1_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=out_9325_1)
    ...
    out_9325_1 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'sector', 'energy-carrier'], aggregation_method='Sum')(df=out_9325_1)
    ...
    # Group by  sector (sum)
    energy_demand_TWh = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'sector'], aggregation_method='Sum')(df=energy_demand_TWh)
    ...
    # energy-production [TWh]
    energy_production_TWh_3 = ExportVariableNode(selected_variable='energy-production[TWh]')(input_table=energy_production_TWh_3)

    # Pathway Explorer
    # - Bio-material demand [t]

    # energy-production [TWh] (only import)
    energy_production_TWh_4 = UseVariableNode(selected_variable='energy-production[TWh]')(input_table=energy_production_TWh_3)
    # Group by  Country, Years, energy-carrier (sum)
    energy_production_TWh_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'energy-carrier'], aggregation_method='Sum')(df=energy_production_TWh_4)
    # energy-imported [TWh]
    out_9509_1 = energy_production_TWh_4.rename(columns={'energy-production[TWh]': 'energy-imported[TWh]'})
    # Module = Scope 2/3
    out_9509_1 = ColumnFilterNode(pattern='^.*$')(df=out_9509_1)
    # Group by  Country, Years (sum)
    energy_production_TWh_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=energy_production_TWh_3)
    # total-energy-supply[TWh] = energy-production[TWh] (other than import) + energy-production[TWh] (import)
    total_energy_supply_TWh = MCDNode(operation_selection='x + y', output_name='total-energy-supply[TWh]')(input_table_1=energy_production_TWh_2, input_table_2=energy_production_TWh_4)
    # pct-bioenergy-produced[%] = energy-production[TWh] (other than import) / total-energy-supply[TWh]
    pct_bioenergy_produced_percent = MCDNode(operation_selection='x / y', output_name='pct-bioenergy-produced[%]')(input_table_1=energy_production_TWh_2, input_table_2=total_energy_supply_TWh)

    # Pathway Explorer
    # - % energy import and % energy produced locally

    # pct-bioenergy-produced [%]
    pct_bioenergy_produced_percent = UseVariableNode(selected_variable='pct-bioenergy-produced[%]')(input_table=pct_bioenergy_produced_percent)
    # pct-bioenergy-imported[%] = energy-production[TWh] (import) / total-energy-supply[TWh]
    pct_bioenergy_imported_percent = MCDNode(operation_selection='x / y', output_name='pct-bioenergy-imported[%]')(input_table_1=energy_production_TWh_4, input_table_2=total_energy_supply_TWh)
    # pct-bioenergy-imported [%]
    pct_bioenergy_imported_percent = UseVariableNode(selected_variable='pct-bioenergy-imported[%]')(input_table=pct_bioenergy_imported_percent)
    ...
    # INDUSTRY
    out_9344_1 = InjectVariablesDataNode()(df=port_01)
    ...
    material_production_Mt = UseVariableNode(selected_variable='material-production[Mt]')(input_table=out_9344_1)
    ...
    material_production_t = ExportVariableNode(selected_variable='material-production[t]')(input_table=material_production_t)
    # material-production [t] (from industry)
    material_production_t = UseVariableNode(selected_variable='material-production[t]')(input_table=material_production_t)
    # Group by  Country, Years, material (sum)
    material_production_t = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'material'], aggregation_method='Sum')(df=material_production_t)
    ...
    out_9342_1 = AddTrigram()(module_name=module_name, df=out_1)
    # Module = Pathway Explorer
    out_9342_1 = ColumnFilterNode(pattern='^.*$')(df=out_9342_1)
    # Keep Country, Years only  (temp solution)
    out_9342_1_2 = ColumnFilterNode(pattern='^Country$|^Years$')(df=out_9342_1)
    # Module = Climate  SHOULD BE DONE ??!!
    out_9342_1_2 = ColumnFilterNode(pattern='^.*$')(df=out_9342_1_2)
    # Years
