...
def metanode_9098(port_01):
    ...
    cropland_management_ha_2 = UseVariableNode(selected_variable='cropland-management[ha]')(input_table=out_5965_1)
    # OTS (only) woody-fruit-ratio [%]
    woody_fruit_ratio_percent = ImportDataNode(trigram='agr', variable_name='woody-fruit-ratio', variable_type='OTS (only)')()
    # Same as last available year
    woody_fruit_ratio_percent = AddMissingYearsNode()(df_data=woody_fruit_ratio_percent)
    # Group by  Country, Years (sum)
    woody_fruit_ratio_percent = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=woody_fruit_ratio_percent)
    # Group by  Country, Years (sum)
    cropland_management_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=cropland_management_ha_2)
    ...
    # Group by  Country, Years (sum)
    cropland_management_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=cropland_management_ha_2)
    # cropland-management[ha] (woody-fruit cropland) = cropland-management[ha] (fruit only) * woody-fruit-ratio[%]
    cropland_management_ha_2 = MCDNode(operation_selection='x * y', output_name='cropland-management[ha]')(input_table_1=cropland_management_ha_2, input_table_2=woody_fruit_ratio_percent)
    # fruit-pct[%] = cropland-management[ha] (woody fruit only) / cropland-management[ha]
    fruit_pct_percent = MCDNode(operation_selection='x / y', output_name='fruit-pct[%]')(input_table_1=cropland_management_ha_2, input_table_2=cropland_management_ha)
    ...
    land_management_ha = UseVariableNode(selected_variable='land-management[ha]')(input_table=out_5965_1)
    ...
    land_management_ha_2 = ImportDataNode(trigram='agr', variable_name='land-management')()
    ...
    total_land_surface_ha = ImportDataNode(trigram='agr', variable_name='total-land-surface', variable_type='OTS (only)')()
    # Same as last available year
    total_land_surface_ha = AddMissingYearsNode()(df_data=total_land_surface_ha)
    # Group by  Country, Years (sum)
    total_land_surface_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=total_land_surface_ha)
    land_management_ha_2 = land_management_ha_2.loc[~land_management_ha_2['land-use'].isin(['frozen-forest'])].copy()
    # Same as last available year
    land_management_ha_2 = AddMissingYearsNode()(df_data=land_management_ha_2)
    ...
    # land-management [ha]
    land_management_ha_2 = ExportVariableNode(selected_variable='land-management[ha]')(input_table=land_management_ha)
    ...
    # land-management [ha]
    land_management_ha = UseVariableNode(selected_variable='land-management[ha]')(input_table=land_management_ha_2)
    # Group by  Country, Years (sum)
    land_management_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=land_management_ha_2)
    # buffer-land[ha] = total-land-surface[ha] - land-management[ha]
    buffer_land_ha = MCDNode(operation_selection='y - x', output_name='buffer-land[ha]')(input_table_1=land_management_ha_2, input_table_2=total_land_surface_ha)
    # ROUND buffer-land[ha] at 3 decimals (avoid accounting for little changes)
    buffer_land_ha_2 = MathFormulaNode(convert_to_int=False, replaced_column='buffer-land[ha]', splitted='round($buffer-land[ha]$, 3)')(df=buffer_land_ha)
    # If < 0 => set 0
    ...
    reforestation_land_man_percent = ImportDataNode(trigram='agr', variable_name='reforestation-land-man')()
    # land-management[ha] = buffer-land[ha] * (1 - reforestation-land-man[%])
    land_management_ha_2 = MCDNode(operation_selection='x * (1-y)', output_name='land-management[ha]')(input_table_1=buffer_land_ha, input_table_2=reforestation_land_man_percent)
    ...
    dynamic_land_man_percent = ImportDataNode(trigram='agr', variable_name='dynamic-land-man')()
    # Group by  Country, Years (sum)
    land_management_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=land_management_ha_2)
    # land-management[ha] (replace) = land-management[ha] * dynamic-land-man [%]
    land_management_ha_2 = MCDNode(operation_selection='x * y', output_name='land-management[ha]')(input_table_1=land_management_ha_2, input_table_2=dynamic_land_man_percent)
    # land-management[ha] = buffer-land[ha] * reforestation-land-man[%]
    land_management_ha_3 = MCDNode(operation_selection='x * y', output_name='land-management[ha]')(input_table_1=buffer_land_ha, input_table_2=reforestation_land_man_percent)
    ...
    # forest-decrease [ha]
    forest_decrease_ha = UseVariableNode(selected_variable='forest-decrease[ha]')(input_table=forest_decrease_ha)
    # land-use = forest
    ...
    land_management_ha = MCDNode(operation_selection='x - y', output_name='land-management[ha]', fill_value_bool='Left [x] Outer Join')(input_table_1=land_management_ha, input_table_2=forest_decrease_ha)
    # land-management[ha] (replace) = land-management[ha] + land-management[ha]  LEFT JOIN If reforestation is missing => Set 0
    land_management_ha = MCDNode(operation_selection='x + y', output_name='land-management[ha]', fill_value_bool='Left [x] Outer Join')(input_table_1=land_management_ha, input_table_2=land_management_ha_3)
    # Add other buffer land (other than forest)
    land_management_ha = pd.concat([land_management_ha, land_management_ha_2.set_index(land_management_ha_2.index.astype(str) + '_dup')])
    # Group by  Country, Years, land-use (sum)
    land_management_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=land_management_ha)
    # land-management [ha]
    ...
    climatic_area_share_percent = ImportDataNode(trigram='agr', variable_name='climatic-area-share', variable_type='RCP')()
    # Remove 0.0  values (to avoid having several  unused lines)
    out_10046_1 = RowFilterNode(filter_type='RangeVal_RowFilter', that_column='climatic-area-share[%]', include=False, lower_bound_bool=True, upper_bound_bool=True, lower_bound='0.0', upper_bound='0.0')(df=climatic_area_share_percent)
    # land-management[ha] (replace) = land-management[ha] * climatic-area-share[%]
    land_management_ha_3 = MCDNode(operation_selection='x * y', output_name='land-management[ha]')(input_table_1=land_management_ha, input_table_2=out_10046_1)
    ...
    land_management_ha_3 = MCDNode(operation_selection='x * y', output_name='land-management[ha]')(input_table_1=fruit_pct_percent, input_table_2=land_management_ha_2)
    ...
    land_management_ha_2 = MCDNode(operation_selection='(1-x) * y', output_name='land-management[ha]')(input_table_1=fruit_pct_percent, input_table_2=land_management_ha_2)
    ...
    agr_land_lifespan_years = ImportDataNode(trigram='agr', variable_name='agr_land-lifespan', variable_type='CP')()
    # Remove source, data_type, module
    agr_land_lifespan_years = ColumnFilterNode(columns_to_drop=['source', 'module', 'data_type'])(df=agr_land_lifespan_years)
    ...
    # new-cum-land [ha]
    new_cum_land_ha = UseVariableNode(selected_variable='new-cum-land[ha]')(input_table=out_688_1)
    ...
    # remaining-land [ha]
    remaining_land_ha = UseVariableNode(selected_variable='remaining-land[ha]')(input_table=out_688_1)
    ...
    # land-area [ha]
    land_area_ha = ExportVariableNode(selected_variable='land-area[ha]')(input_table=out_1)
    ...
    # land-area [ha]
    land_area_ha = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha)
    ...
    # land-area [ha]
    land_area_ha_3 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha)
    ...
    land_area_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'ecological-zone', 'sub-climate-type'], aggregation_method='Sum')(df=land_area_ha_2)
    ...
    # land-area [ha]
    land_area_ha_4 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_3)

    # Global parameters

    # CP organic-soil-emission-factor (agr_soil-organic-ef_cp) [tC/ha]
    agr_soil_organic_ef_tC_per_ha = ImportDataNode(trigram='agr', variable_name='agr_soil-organic-ef', variable_type='CP')()
    # Remove unused columns (source, module, data-type) (mean)
    agr_soil_organic_ef_tC_per_ha = GroupByDimensions(groupby_dimensions=['climate-type', 'land-use'], aggregation_method='Mean')(df=agr_soil_organic_ef_tC_per_ha)
    ...
    agr_soil_organic_ef_tC_per_ha_excluded = GroupByDimensions(groupby_dimensions=['climate-type'], aggregation_method='Sum')(df=agr_soil_organic_ef_tC_per_ha_excluded_2)
    ...
    # Group by climate-type (sum)
    agr_soil_organic_ef_tC_per_ha_excluded_excluded = GroupByDimensions(groupby_dimensions=['climate-type'], aggregation_method='Sum')(df=agr_soil_organic_ef_tC_per_ha_excluded_excluded)
    ...
    land_area_ha_4 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_excluded_excluded)
    # Group by  Country, Years, land-use, land-age, climate-type (sum)
    land_area_ha_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'land-age'], aggregation_method='Sum')(df=land_area_ha_4)
    # land-area [ha] GRASSLAND only
    land_area_ha_5 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_excluded_2)
    # Group by  Country, Years, land-use, land-age, climate-type (sum)
    land_area_ha_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'land-age'], aggregation_method='Sum')(df=land_area_ha_5)

    # Carbon dynamics - 3.A) Organic soils

    # land-area [ha]  FOREST only
    land_area_ha_3 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_3)
    # RCP organic-area-share [%]
    organic_area_share_percent = ImportDataNode(trigram='agr', variable_name='organic-area-share', variable_type='RCP')()
    ...
    # organic-area-share [%]
    organic_area_share_percent_4 = UseVariableNode(selected_variable='organic-area-share[%]')(input_table=organic_area_share_percent)
    # Keep pasture
    ...
    organic_area_share_percent_excluded = GroupByDimensions(groupby_dimensions=['Country'], aggregation_method='Mean')(df=organic_area_share_percent_excluded)
    ...
    # organic-area-share [%]
    organic_area_share_percent_3 = UseVariableNode(selected_variable='organic-area-share[%]')(input_table=organic_area_share_percent_excluded)
    # organic-land-area[ha] = land-area[ha] * drained-area-share[%]
    organic_land_area_ha = MCDNode(operation_selection='x * y', output_name='organic-land-area[ha]')(input_table_1=land_area_ha_4, input_table_2=organic_area_share_percent_excluded)
    # carbon-change[tC] = organic-land-area[ha] * organic-soil-emission-factor [tC/ha]  Change = losses (here)
    carbon_change_tC = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=organic_land_area_ha, input_table_2=agr_soil_organic_ef_tC_per_ha_excluded_excluded)
    # * (-1) (we loose C-stock)
    carbon_change_tC['carbon-change[tC]'] = carbon_change_tC['carbon-change[tC]']*(-1.0)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC)
    # c-change-origin = organic-soils
    carbon_change_tC['c-change-origin'] = "organic-soils"
    # All except land-use (mean)
    organic_area_share_percent_2 = GroupByDimensions(groupby_dimensions=['Country'], aggregation_method='Mean')(df=organic_area_share_percent_2)
    ...
    # organic-area-share [%]
    organic_area_share_percent_5 = UseVariableNode(selected_variable='organic-area-share[%]')(input_table=organic_area_share_percent_2)
    # organic-land-area[ha] = land-area[ha] * drained-area-share[%]
    organic_land_area_ha = MCDNode(operation_selection='x * y', output_name='organic-land-area[ha]')(input_table_1=land_area_ha_5, input_table_2=organic_area_share_percent_2)
    # carbon-change[tC] = organic-land-area[ha] * organic-soil-emission-factor [tC/ha]  Change = losses (here)
    carbon_change_tC_2 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=organic_land_area_ha, input_table_2=agr_soil_organic_ef_tC_per_ha_excluded)
    # * (-1) (we loose C-stock)
    carbon_change_tC_2['carbon-change[tC]'] = carbon_change_tC_2['carbon-change[tC]']*(-1.0)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_2)
    # c-change-origin = organic-soils
    ...
    # Group by  Country, Years, land-use, land-age, climate-type (sum)
    land_area_ha_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'land-age'], aggregation_method='Sum')(df=land_area_ha_3)
    # organic-land-area[ha] = land-area[ha] * drained-area-share[%]
    organic_land_area_ha = MCDNode(operation_selection='x * y', output_name='organic-land-area[ha]')(input_table_1=land_area_ha_3, input_table_2=organic_area_share_percent)
    # carbon-change[tC] = organic-land-area[ha] * organic-soil-emission-factor [tC/ha]  Change = losses (here)
    carbon_change_tC_2 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=organic_land_area_ha, input_table_2=agr_soil_organic_ef_tC_per_ha_2)
    # * (-1) (we loose C-stock)
    carbon_change_tC_2['carbon-change[tC]'] = carbon_change_tC_2['carbon-change[tC]']*(-1.0)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_2)
    ...
    agr_above_ground_net_biomass_growth_tdm_per_ha_per_yr = ImportDataNode(trigram='agr', variable_name='agr_above-ground-net-biomass-growth', variable_type='CP')()
    # RCP forest-above-ground-biomass [tdm/ha]
    forest_above_ground_biomass_tdm_per_ha = ImportDataNode(trigram='agr', variable_name='forest-above-ground-biomass', variable_type='RCP')()
    # RCP forest-above-to-below-biomass -ratio [tdm/tdm]
    forest_above_to_below_biomass_ratio_tdm_per_tdm = ImportDataNode(trigram='agr', variable_name='forest-above-to-below-biomass-ratio', variable_type='RCP')()
    ...
    # biomass-ratio [%]
    biomass_ratio_percent = UseVariableNode(selected_variable='biomass-ratio[%]')(input_table=biomass_ratio_percent)
    # biomass-ratio [%]
    biomass_ratio_percent_2 = UseVariableNode(selected_variable='biomass-ratio[%]')(input_table=biomass_ratio_percent)

    # Carbon dynamics - 1.C) Biomass : Losses due to wood removal and fuel wood
    # Note : We use above ground-biomass growth to avoid extracting more than the forest regeneration

    # biomass-ratio [%]
    biomass_ratio_percent_2 = UseVariableNode(selected_variable='biomass-ratio[%]')(input_table=biomass_ratio_percent_2)
    ...
    ground_biomass_tdm_per_ha = MCDNode(operation_selection='x * y', output_name='ground-biomass[tdm/ha]')(input_table_1=forest_above_ground_biomass_tdm_per_ha, input_table_2=biomass_ratio_percent)

    # Carbon dynamics - 1.B) Biomass : Losses due to disturbance

    # ground-biomass [tdm/ha]
    ground_biomass_tdm_per_ha = UseVariableNode(selected_variable='ground-biomass[tdm/ha]')(input_table=ground_biomass_tdm_per_ha)
    # Group by  climact-type, land-purpose, ecological-zone (sum)
    agr_above_ground_net_biomass_growth_tdm_per_ha_per_yr = GroupByDimensions(groupby_dimensions=['climate-type', 'ecological-zone', 'land-purpose'], aggregation_method='Sum')(df=agr_above_ground_net_biomass_growth_tdm_per_ha_per_yr)
    # ground-net-biomass-growth[tdm/ha] = above-ground-net-biomass-growth [tdm/ha/yr] * biomass-ratio[%]
    ground_net_biomass_growth_tdm_per_ha = MCDNode(operation_selection='x * y', output_name='ground-net-biomass-growth[tdm/ha]')(input_table_1=agr_above_ground_net_biomass_growth_tdm_per_ha_per_yr, input_table_2=biomass_ratio_percent)

    # Carbon dynamics - 1.A) Biomass : Gains

    # ground-net-biomass-growth [tdm/ha]
    ground_net_biomass_growth_tdm_per_ha = UseVariableNode(selected_variable='ground-net-biomass-growth[tdm/ha]')(input_table=ground_net_biomass_growth_tdm_per_ha)
    # above-ground-net-biomass-growth [tdm/ha/yr]
    above_ground_net_biomass_growth_tdm_per_ha_per_yr = UseVariableNode(selected_variable='above-ground-net-biomass-growth[tdm/ha/yr]')(input_table=agr_above_ground_net_biomass_growth_tdm_per_ha_per_yr)
    # above-ground-net-biomass-growth [tdm/ha/yr]
    above_ground_net_biomass_growth_tdm_per_ha_per_yr = UseVariableNode(selected_variable='above-ground-net-biomass-growth[tdm/ha/yr]')(input_table=above_ground_net_biomass_growth_tdm_per_ha_per_yr)
    # OTS/FTS wood-harvest [%]
    wood_harvest_percent = ImportDataNode(trigram='agr', variable_name='wood-harvest')()
    # wood-harvest[tdm/ha] = above-ground-net-biomass-growth [tdm/ha/yr] * wood-harvest[%]
    wood_harvest_tdm_per_ha = MCDNode(operation_selection='x * y', output_name='wood-harvest[tdm/ha]')(input_table_1=above_ground_net_biomass_growth_tdm_per_ha_per_yr, input_table_2=wood_harvest_percent)
    # Keep forests land-area[ha]
    ...
    cover_area_share_percent = ImportDataNode(trigram='agr', variable_name='cover-area-share', variable_type='OTS (only)')()
    # Same as last available year
    cover_area_share_percent = AddMissingYearsNode()(df_data=cover_area_share_percent)
    # cover-area-share [%]
    cover_area_share_percent_2 = UseVariableNode(selected_variable='cover-area-share[%]')(input_table=cover_area_share_percent)
    # annual-new-land[ha] (replace) = annual-new-land[ha] * cover-area-share[%]
    annual_new_land_ha = MCDNode(operation_selection='x * y', output_name='annual-new-land[ha]')(input_table_1=land_area_ha_2, input_table_2=cover_area_share_percent_2)

    # Carbon dynamics - 2) DOM (only on changes (new / lost land) - cfr IPCC Guidances)

    # annual-new-land [ha] FOREST only
    annual_new_land_ha = UseVariableNode(selected_variable='annual-new-land[ha]')(input_table=annual_new_land_ha)
    # / 20 (Ton = 20)
    annual_new_land_ha['annual-new-land[ha]'] = annual_new_land_ha['annual-new-land[ha]']/20.0
    # OTS (only) purpose-area-share [%]
    purpose_area_share_percent = ImportDataNode(trigram='agr', variable_name='purpose-area-share', variable_type='OTS (only)')()
    # Same as last available year
    purpose_area_share_percent = AddMissingYearsNode()(df_data=purpose_area_share_percent)
    # area-share[%] (replace) = purpose-area-share[%] * cover-area-share[%]
    area_share_percent = MCDNode(operation_selection='x * y', output_name='area-share[%]')(input_table_1=cover_area_share_percent, input_table_2=purpose_area_share_percent)
    # land-area[ha] (replace) = land-area[ha] * cover-area-share[%]
    land_area_ha = MCDNode(operation_selection='x * y', output_name='land-area[ha]')(input_table_1=land_area_ha_3, input_table_2=area_share_percent)
    # Top : temperate
    ...
    land_area_ha_excluded_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'ecological-zone', 'sub-climate-type', 'land-age', 'land-purpose'], aggregation_method='Sum')(df=land_area_ha_excluded_2)
    ...
    land_area_ha = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha)
    # land-area [ha]
    land_area_ha_2 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha)
    # OTS/FTS disturbed-area [%]
    disturbed_area_percent = ImportDataNode(trigram='agr', variable_name='disturbed-area')()
    # disturbed-land-area[ha] = land-area[ha] * disturbed-area[%]
    disturbed_land_area_ha = MCDNode(operation_selection='x * y', output_name='disturbed-land-area[ha]')(input_table_1=land_area_ha_2, input_table_2=disturbed_area_percent)
    # disturbed-above-ground-biomass[tdm] = disturbed-land-area[ha] * ground-biomass [tdm/ha]
    disturbed_above_ground_biomass_tdm = MCDNode(operation_selection='x * y', output_name='disturbed-above-ground-biomass[tdm]')(input_table_1=disturbed_land_area_ha, input_table_2=ground_biomass_tdm_per_ha)
    # land-area [ha]
    land_area_ha_2 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_2)
    # removal-wood[tdm] = land-area[ha] * wood-harvest[tdm/ha]
    removal_wood_tdm = MCDNode(operation_selection='x * y', output_name='removal-wood[tdm]')(input_table_1=land_area_ha_2, input_table_2=wood_harvest_tdm_per_ha)

    # Calibration :
    # On wood harvest [tdm]

    # removal-biomass[tdm] = removal-wood[tdm] * biomass-ratio[%]
    removal_biomass_tdm = MCDNode(operation_selection='x * y', output_name='removal-biomass[tdm]')(input_table_1=removal_wood_tdm, input_table_2=biomass_ratio_percent_2)
    # Group by Country, Years, land-use (sum)
    removal_biomass_tdm_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=removal_biomass_tdm)
    # Calibration total-removal-wood [tdm]
    total_removal_wood_tdm = ImportDataNode(trigram='agr', variable_name='total-removal-wood', variable_type='Calibration')()
    # Apply Calibration on wood-harvest[tdm]
    _, out_10036_2, _ = CalibrationNode(data_to_be_cal='removal-biomass[tdm]', data_cal='total-removal-wood[tdm]')(input_table=removal_biomass_tdm_2, cal_table=total_removal_wood_tdm)
    # removal-biomass[tdm] (replace) = removal-biomass[tdm] * cal-rate
    removal_biomass_tdm_2 = MCDNode(operation_selection='x * y', output_name='removal-biomass[tdm]')(input_table_1=removal_biomass_tdm, input_table_2=out_10036_2)
    ...
    removal_biomass_tdm = UseVariableNode(selected_variable='removal-biomass[tdm]')(input_table=removal_biomass_tdm_2)
    # OTS (only) total-wood-production [m3]
    total_wood_production_m3 = ImportDataNode(trigram='agr', variable_name='total-wood-production', variable_type='OTS (only)')()
    # conversion-factor[tdm/m3] = removal-biomass[tdm] / total-wood-production[m3]
    conversion_factor_tdm_per_m3 = MCDNode(operation_selection='x / y', output_name='conversion-factor[tdm/m3]')(input_table_1=removal_biomass_tdm, input_table_2=total_wood_production_m3)
    # Same as last available year (we assume we have  the same "yield" in the future)
    conversion_factor_tdm_per_m3 = AddMissingYearsNode()(df_data=conversion_factor_tdm_per_m3)
    # wood-production[m3] = removal-biomass[tdm] / conversion-factor[tdm/m3]
    wood_production_m3 = MCDNode(operation_selection='x / y', output_name='wood-production[m3]')(input_table_1=removal_biomass_tdm, input_table_2=conversion_factor_tdm_per_m3)
    # Group by Country, Years, land-cover, land-purpose
    wood_production_m3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-cover', 'land-purpose'], aggregation_method='Sum')(df=wood_production_m3)
    # wood-production [m3]
    wood_production_m3 = ExportVariableNode(selected_variable='wood-production[m3]')(input_table=wood_production_m3)
    ...
    # wood-production [m3]
    wood_production_m3_2 = UseVariableNode(selected_variable='wood-production[m3]')(input_table=wood_production_m3)
    ...
    above_ground_biomass_tdm = MCDNode(operation_selection='x * y', output_name='above-ground-biomass[tdm]')(input_table_1=land_area_ha, input_table_2=ground_net_biomass_growth_tdm_per_ha)
    # CP (agr_)carbon-fraction [tC/tdm]
    agr_carbon_fraction_tC_per_tdm = ImportDataNode(trigram='agr', variable_name='agr_carbon-fraction', variable_type='CP')()
    # Group by climate-type (mean) (Remove unused columns)
    agr_carbon_fraction_tC_per_tdm = GroupByDimensions(groupby_dimensions=['climate-type'], aggregation_method='Mean')(df=agr_carbon_fraction_tC_per_tdm)
    # carbon-change[tC] = above-ground-biomass[tdm] * carbon-fraction[tC/tdm]  Change = gains (here)
    carbon_change_tC_2 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=above_ground_biomass_tdm, input_table_2=agr_carbon_fraction_tC_per_tdm)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_2)
    ...
    carbon_change_tC_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age', 'c-change-origin'], aggregation_method='Sum')(df=carbon_change_tC_2)
    # carbon-fraction [tC/tdm]
    carbon_fraction_tC_per_tdm = UseVariableNode(selected_variable='carbon-fraction[tC/tdm]')(input_table=agr_carbon_fraction_tC_per_tdm)
    # carbon-change[tC] = removal-biomass[tdm] * carbon-fraction[tC/tdm]  Change = losses (here)
    carbon_change_tC_3 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=carbon_fraction_tC_per_tdm, input_table_2=removal_biomass_tdm_2)
    # * (-1) (we loose C-stock)
    carbon_change_tC_3['carbon-change[tC]'] = carbon_change_tC_3['carbon-change[tC]']*(-1.0)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_3 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_3)

    # Calibration :
    # On carbon change [tC]

    # Group by Country, Years, land-use (sum)
    carbon_change_tC_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=carbon_change_tC_3)
    # Calibration wood-harvest-emissions [tCO2]
    wood_harvest_emissions_tCO2 = ImportDataNode(trigram='agr', variable_name='wood-harvest-emissions', variable_type='Calibration')()
    ...
    _, out_10105_2, _ = CalibrationNode(data_to_be_cal='carbon-change[tC]', data_cal='wood-harvest-emissions[tC]')(input_table=carbon_change_tC_4, cal_table=wood_harvest_emissions_tC)
    # carbon-change[tC] = carbon-change[tC] * cal-rate 
    carbon_change_tC_3 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=carbon_change_tC_3, input_table_2=out_10105_2)
    ...
    carbon_stock_tC = MCDNode(operation_selection='x * y', output_name='carbon-stock[tC]')(input_table_1=carbon_fraction_tC_per_tdm, input_table_2=disturbed_above_ground_biomass_tdm)
    # CP disturbance-lost-factor (agr_disturbance) [%]
    agr_disturbance_percent = ImportDataNode(trigram='agr', variable_name='agr_disturbance', variable_type='CP')()
    # Group by  disturbance-type (mean) (remove unused columns)
    agr_disturbance_percent = GroupByDimensions(groupby_dimensions=['disturbance-type'], aggregation_method='Mean')(df=agr_disturbance_percent)
    # carbon-change[tC] = carbon-stock[tC] * disturbance-lost-factor[%]  Change = losses (here)
    carbon_change_tC_4 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=carbon_stock_tC, input_table_2=agr_disturbance_percent)
    # * (-1) (we loose C-stock)
    carbon_change_tC_4['carbon-change[tC]'] = carbon_change_tC_4['carbon-change[tC]']*(-1.0)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_4 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_4)
    ...
    # land-area [ha]  (forest only)
    land_area_ha = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_3)
    # Keep land-age = new
    land_area_ha = land_area_ha.loc[land_area_ha['land-age'].isin(['new'])].copy()
    # Group by Country, Years, land-use (sum)
    land_area_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=land_area_ha)
    ...
    out_10134_1 = SpreadCapital()(output_table=capex_new_forest_MEUR, df_wacc=wacc_percent)
    # capex-new-forests [MEUR]
    capex_new_forest_MEUR = ExportVariableNode(selected_variable='capex-new-forest[MEUR]')(input_table=out_10134_1)
    # Keep Years >= baseyear
    capex_new_forest_MEUR, _ = FilterDimension(dimension='Years', operation_selection='≥', value_years=Globals.get().base_year)(df=capex_new_forest_MEUR)
    # Keep grassland(s) land-area[ha]
    land_area_ha_excluded_2 = land_area_ha_excluded.loc[land_area_ha_excluded['land-use'].isin(['natural-prairies', 'pasture'])].copy()
    land_area_ha_excluded_excluded = land_area_ha_excluded.loc[~land_area_ha_excluded['land-use'].isin(['natural-prairies', 'pasture'])].copy()
    ...
    grassland_above_ground_biomass_tdm_per_ha = ImportDataNode(trigram='agr', variable_name='grassland-above-ground-biomass', variable_type='RCP')()
    # RCP grassland-above-to-below-biomass -ratio [tdm/tdm]
    grassland_above_to_below_biomass_ratio_tdm_per_tdm = ImportDataNode(trigram='agr', variable_name='grassland-above-to-below-biomass-ratio', variable_type='RCP')()
    ...
    ground_biomass_tdm_per_ha = MCDNode(operation_selection='x * y', output_name='ground-biomass[tdm/ha]')(input_table_1=grassland_above_ground_biomass_tdm_per_ha, input_table_2=biomass_ratio_percent)
    ...
    ground_biomass_tdm_per_ha = UseVariableNode(selected_variable='ground-biomass[tdm/ha]')(input_table=ground_biomass_tdm_per_ha)
    # land-area [ha]
    land_area_ha = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_excluded_2)
    # Keep only new land (=cum new) 
    land_area_ha = land_area_ha.loc[land_area_ha['land-age'].isin(['new'])].copy()
    # Group by  Country, Years, land-use, climate, sub-climate (sum)
    land_area_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type'], aggregation_method='Sum')(df=land_area_ha)
    # above-ground-biomass[tdm] = land-change[ha] * ground-net-biomass-growth [tdm/ha]
    above_ground_biomass_tdm_2 = MCDNode(operation_selection='x * y', output_name='above-ground-biomass[tdm]')(input_table_1=land_area_ha, input_table_2=ground_biomass_tdm_per_ha)
    ...
    settlement_tree_cover_percent = ImportDataNode(trigram='agr', variable_name='settlement-tree-cover', variable_type='OTS (only)')()
    # Same as last available year
    settlement_tree_cover_percent = AddMissingYearsNode()(df_data=settlement_tree_cover_percent)
    # land-area[ha] (replace) = land-area[ha] * settlement-tree-cover[%]
    land_area_ha = MCDNode(operation_selection='x * y', output_name='land-area[ha]')(input_table_1=land_area_ha_excluded_excluded_excluded, input_table_2=settlement_tree_cover_percent)
    ...
    biomass_yields_tons_per_ha = ImportDataNode(trigram='agr', variable_name='biomass-yields', variable_type='OTS (only)')()
    # Same as last available year
    biomass_yields_tons_per_ha = AddMissingYearsNode()(df_data=biomass_yields_tons_per_ha)
    # Group by Country, Years, land-use (sum)
    land_area_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=land_area_ha_2)
    # potential-biomass-production[t] = land-area[ha] * biomass-yields[t/ha]
    potential_biomass_production_t = MCDNode(operation_selection='x * y', output_name='potential-biomass-production[t]')(input_table_1=land_area_ha_2, input_table_2=biomass_yields_tons_per_ha)
    ...
    biomass_bioenergy_share_percent = ImportDataNode(trigram='agr', variable_name='biomass-bioenergy-share')()
    # biomass-harvested[t] = potential-biomass-production[t] * biomass-bioenergy-share[%]
    biomass_harvested_t = MCDNode(operation_selection='x * y', output_name='biomass-harvested[t]')(input_table_1=potential_biomass_production_t, input_table_2=biomass_bioenergy_share_percent)
    # biomass-harvested [t]
    biomass_harvested_t = ExportVariableNode(selected_variable='biomass-harvested[t]')(input_table=biomass_harvested_t)

    # Settlement => green wastes (élagage, ...)

    # biomass-harvested [t]
    biomass_harvested_t = UseVariableNode(selected_variable='biomass-harvested[t]')(input_table=biomass_harvested_t)
    # CP land-management-bioenergy-conv-factors [TWh/ton]
    land_management_bioenergy_conv_factors_TWh_per_ton = ImportDataNode(trigram='agr', variable_name='land-management-bioenergy-conv-factors', variable_type='RCP')()
    # energy-production[TWh] = biomass-harvested[t] * land-management-bioenergy-conv-factors [TWh/ton]
    energy_production_TWh = MCDNode(operation_selection='x * y', output_name='energy-production[TWh]')(input_table_1=biomass_harvested_t, input_table_2=land_management_bioenergy_conv_factors_TWh_per_ton)
    ...
    land_area_ha_2 = UseVariableNode(selected_variable='land-area[ha]')(input_table=land_area_ha_excluded_excluded)
    # Group by Country, Years, land-use (sum)
    land_area_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=land_area_ha_2)
    # OTS (only) energy-crop-yields [tons/ha]
    energy_crop_yields_tons_per_ha = ImportDataNode(trigram='agr', variable_name='energy-crop-yields', variable_type='OTS (only)')()
    # Same as last available year
    energy_crop_yields_tons_per_ha = AddMissingYearsNode()(df_data=energy_crop_yields_tons_per_ha)
    # potential-biomass-production[t] = land-area[ha] * energy-crops-yields[t/ha]  We consider all is harvested
    potential_biomass_production_t = MCDNode(operation_selection='x * y', output_name='potential-biomass-production[t]')(input_table_1=land_area_ha_2, input_table_2=energy_crop_yields_tons_per_ha)
    # potential-biomass-production [t]
    potential_biomass_production_t = ExportVariableNode(selected_variable='potential-biomass-production[t]')(input_table=potential_biomass_production_t)
    ...
    # potential-biomass-production [t]
    potential_biomass_production_t = UseVariableNode(selected_variable='potential-biomass-production[t]')(input_table=potential_biomass_production_t)

    # Apply import-energy-crops levers (switch)
    # => determine % of energy crops that are imported / exported

    # OTS/FTS energy-crops-share [%]
    energy_crops_share_percent = ImportDataNode(trigram='agr', variable_name='energy-crops-share')()
    # potential-production-for-industry[t] = potential-biomass-production[t] * (1 - energy-crops-share[%])
    potential_production_for_industry_t = MCDNode(operation_selection='x * (1-y)', output_name='potential-production-for-industry[t]')(input_table_1=potential_biomass_production_t, input_table_2=energy_crops_share_percent)
    # origin = industrial-crops
    potential_production_for_industry_t['origin'] = "industrial-crops"
    # potential-production-for-industry [t]
    potential_production_for_industry_t = ExportVariableNode(selected_variable='potential-production-for-industry[t]')(input_table=potential_production_for_industry_t)
    ...
    # potential-production-for-industry [t]
    potential_production_for_industry_t = UseVariableNode(selected_variable='potential-production-for-industry[t]')(input_table=potential_production_for_industry_t)
    ...
    potential_biomass_production_t = MCDNode(operation_selection='x * y', output_name='potential-biomass-production[t]')(input_table_1=potential_biomass_production_t, input_table_2=energy_crops_share_percent)
    # RCP energy-crops-conversion-factor [TWh/t]
    energy_crops_conversion_factor_TWh_per_t = ImportDataNode(trigram='agr', variable_name='energy-crops-conversion-factor', variable_type='RCP')()
    # energy-production[TWh] = potentiel-biomass-production[t] * energy-crops-conversion-factor [TWh/t]
    energy_production_TWh_2 = MCDNode(operation_selection='x * y', output_name='energy-production[TWh]')(input_table_1=potential_biomass_production_t, input_table_2=energy_crops_conversion_factor_TWh_per_t)
    # Group by  Country, Years, energy-carrier (sum)
    ...
    # Module = Bioenergy
    production = ColumnFilterNode(pattern='^.*$')(df=production)
    ...
    biomass_accumulation_rate_tC_per_ha_per_yr = ImportDataNode(trigram='agr', variable_name='biomass-accumulation-rate', variable_type='RCP')()
    ...
    biomass_accumulation_rate_tC_per_ha_per_yr_2 = UseVariableNode(selected_variable='biomass-accumulation-rate[tC/ha/yr]')(input_table=biomass_accumulation_rate_tC_per_ha_per_yr)
    ...
    biomass_accumulation_rate_tC_per_ha_per_yr_2 = GroupByDimensions(groupby_dimensions=['Country', 'climate-type', 'sub-climate-type'], aggregation_method='Sum')(df=biomass_accumulation_rate_tC_per_ha_per_yr_2)
    # carbon-change[tC] = land-area[ha] (only new lands) * biomass-accumulation-rate [tC/ha/yr]  Change = gains (here)
    carbon_change_tC_7 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=land_area_ha, input_table_2=biomass_accumulation_rate_tC_per_ha_per_yr_2)
    # carbon-change[tC] = land-area[ha] (only new lands) * biomass-accumulation-rate [tC/ha/yr]  Change = gains (here)
    carbon_change_tC_6 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=land_area_ha_excluded_excluded, input_table_2=biomass_accumulation_rate_tC_per_ha_per_yr)
    # annual-lost-land [ha]
    annual_lost_land_ha = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=out_688_1)
    ...
    # annual-lost-land [ha]
    annual_lost_land_ha_2 = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=annual_lost_land_ha_2)
    # Group by  Country, Years, land-use, climate, sub-climate (sum)
    annual_lost_land_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type'], aggregation_method='Sum')(df=annual_lost_land_ha_2)
    # above-ground-biomass[tdm] = annual-lost-land[ha] * ground-net-biomass-growth [tdm/ha]
    above_ground_biomass_tdm = MCDNode(operation_selection='x * y', output_name='above-ground-biomass[tdm]')(input_table_1=annual_lost_land_ha_2, input_table_2=ground_biomass_tdm_per_ha)
    ...
    # carbon-change[tC]
    carbon_change_tC_5 = UseVariableNode(selected_variable='carbon-change[tC]')(input_table=carbon_change_tC_5)
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_5)
    # c-change-origin = biomass-gains
    carbon_change_tC_5['c-change-origin'] = "biomass-gains"
    # Group by  Country, Years, land-use, land-age (sum)  DO WE WANT TO KEEP THE ORIGIN ?
    carbon_change_tC_5 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age', 'c-change-origin'], aggregation_method='Sum')(df=carbon_change_tC_5)
    ...
    annual_lost_land_ha_2 = MCDNode(operation_selection='x * y', output_name='annual-lost-land[ha]')(input_table_1=annual_lost_land_ha_excluded_excluded, input_table_2=settlement_tree_cover_percent)
    # annual-lost-land [ha]
    annual_lost_land_ha_3 = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=annual_lost_land_ha_2)
    # annual-lost-land [ha]
    annual_lost_land_ha_2 = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=annual_lost_land_ha_excluded_2)
    # RCP biomass-carbon-loss [tC/ha/yr]
    biomass_carbon_loss_tC_per_ha_per_yr = ImportDataNode(trigram='agr', variable_name='biomass-carbon-loss', variable_type='RCP')()
    # biomass-carbon-loss [tC/ha/yr]
    biomass_carbon_loss_tC_per_ha_per_yr_2 = UseVariableNode(selected_variable='biomass-carbon-loss[tC/ha/yr]')(input_table=biomass_carbon_loss_tC_per_ha_per_yr)
    # Keep land-use = wood-cropland
    ...
    # Group by all dimensions except land-use (sum)
    biomass_carbon_loss_tC_per_ha_per_yr_2 = GroupByDimensions(groupby_dimensions=['Country', 'climate-type', 'sub-climate-type'], aggregation_method='Sum')(df=biomass_carbon_loss_tC_per_ha_per_yr_2)
    ...
    carbon_change_tC_8 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=annual_lost_land_ha_3, input_table_2=biomass_carbon_loss_tC_per_ha_per_yr_2)
    # land-age = new (<= 20 years)
    ...
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_7 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_7)
    # c-change-origin = biomass-gains
    carbon_change_tC_7['c-change-origin'] = "biomass-gains"
    # Group by  Country, Years, land-use, land-age (sum)  DO WE WANT TO KEEP THE ORIGIN ?
    carbon_change_tC_7 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age', 'c-change-origin'], aggregation_method='Sum')(df=carbon_change_tC_7)
    # carbon-change[tC] = annual-lost-land[ha] * biomass-carbon-loss [tC/ha/yr]  Change = losses (here) alreadynegative values as lost land are negative
    carbon_change_tC_8 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=annual_lost_land_ha_2, input_table_2=biomass_carbon_loss_tC_per_ha_per_yr)
    # land-age = new (<= 20 years)
    carbon_change_tC_8['land-age'] = "new"
    carbon_change_tC_6 = pd.concat([carbon_change_tC_6, carbon_change_tC_8.set_index(carbon_change_tC_8.index.astype(str) + '_dup')])
    # Group by  Country, Years, land-use, land-age (sum)
    carbon_change_tC_6 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age'], aggregation_method='Sum')(df=carbon_change_tC_6)
    # c-change-origin = biomass-gains
    carbon_change_tC_6['c-change-origin'] = "biomass-gains"
    # Group by  Country, Years, land-use, land-age (sum)  DO WE WANT TO KEEP THE ORIGIN ?
    carbon_change_tC_6 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'land-age', 'c-change-origin'], aggregation_method='Sum')(df=carbon_change_tC_6)
    ...
    # annual-lost-land [ha]
    annual_lost_land_ha = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=annual_lost_land_ha)
    ...
    annual_lost_land_ha = MCDNode(operation_selection='x * y', output_name='annual-lost-land[ha]')(input_table_1=annual_lost_land_ha, input_table_2=cover_area_share_percent_2)
    # annual-lost-land [ha] FOREST only
    annual_lost_land_ha = UseVariableNode(selected_variable='annual-lost-land[ha]')(input_table=annual_lost_land_ha)
    # land-change[ha] = annual-new-land[ha] + annual-lost-land[ha]
    land_change_ha = MCDNode(operation_selection='x + y', output_name='land-change[ha]')(input_table_1=annual_new_land_ha, input_table_2=annual_lost_land_ha)
    # CP (agr_)litter-carbon-stock [tC/ha]
    agr_litter_carbon_stock_tC_per_ha = ImportDataNode(trigram='agr', variable_name='agr_litter-carbon-stock', variable_type='CP')()
    # Remove unused columns (source, module, data-type) (mean)
    agr_litter_carbon_stock_tC_per_ha = GroupByDimensions(groupby_dimensions=['climate-type', 'sub-climate-type', 'land-use', 'land-cover'], aggregation_method='Mean')(df=agr_litter_carbon_stock_tC_per_ha)
    # carbon-change[tC] = land-change[ha] * litter-carbon-stock[tC/ha]
    carbon_change_tC_3 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=land_change_ha, input_table_2=agr_litter_carbon_stock_tC_per_ha)
    # land-age = new (<= 20 years)
    ...
    land_management_lifetime_lagged_ha = UseVariableNode(selected_variable='land-management_lifetime-lagged[ha]')(input_table=out_688_1)
    # land-management- lifetime-lagged [ha]
    land_management_lifetime_lagged_ha = UseVariableNode(selected_variable='land-management_lifetime-lagged[ha]')(input_table=land_management_lifetime_lagged_ha)

    # Add caracteristics linked to Mineral soil computation

    # RCP soil-area-share [%]
    soil_area_share_percent = ImportDataNode(trigram='agr', variable_name='soil-area-share', variable_type='RCP')()
    # land-management- lifetime-lagged [ha] (replace) = land-management- lifetime-lagged [ha] * soil-area-share[%]
    land_management_lifetime_lagged_ha = MCDNode(operation_selection='x * y', output_name='land-management-lifetime-lagged[ha]')(input_table_1=land_management_lifetime_lagged_ha, input_table_2=soil_area_share_percent)
    ...
    land_management_lifetime_lagged_ha_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_lifetime_lagged_ha_excluded_2)
    ...
    land_management_lifetime_lagged_ha_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_lifetime_lagged_ha_excluded_excluded_2)
    ...
    # Group by  Country, Years, land-use, climate-type, sub-climate-type (sum)
    land_management_lifetime_lagged_ha_excluded_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_lifetime_lagged_ha_excluded_excluded_excluded)
    # Group by  Country, Years, land-use, climate-type, sub-climate-type, soil-type (sum)
    land_management_lifetime_lagged_ha = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_lifetime_lagged_ha_2)
    # land-management [ha]
    land_management_ha_2 = UseVariableNode(selected_variable='land-management[ha]')(input_table=out_688_1)
    # land-management [ha]
    ...
    land_management_ha_2 = MCDNode(operation_selection='x * y', output_name='land-management[ha]')(input_table_1=land_management_ha_2, input_table_2=soil_area_share_percent)
    ...
    # Group by  Country, Years, land-use, climate-type, sub-climate-type, soil-type (sum)
    land_management_ha_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_ha_excluded_2)
    # land-change[ha] (based on a 20 year lifespan) = land-management [ha] - land-management- lifetime-lagged [ha]
    land_change_ha = MCDNode(operation_selection='x - y', output_name='land-change[ha]')(input_table_1=land_management_ha_excluded, input_table_2=land_management_lifetime_lagged_ha_excluded)
    # mineral-land-change[ha] = land-change[ha] * (1-organic-area-share[%]
    mineral_land_change_ha = MCDNode(operation_selection='x * (1-y)', output_name='mineral-land-change[ha]')(input_table_1=land_change_ha, input_table_2=organic_area_share_percent_5)
    ...
    # Group by  Country, Years, land-use, climate-type, sub-climate-type (sum)
    land_management_ha_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_ha_excluded_excluded_2)
    # land-change[ha] (based on a 20 year lifespan) = land-management [ha] - land-management- lifetime-lagged [ha]
    land_change_ha = MCDNode(operation_selection='x - y', output_name='land-change[ha]')(input_table_1=land_management_ha_excluded_excluded, input_table_2=land_management_lifetime_lagged_ha_excluded_excluded)
    # mineral-land-change[ha] = land-change[ha] * (1-organic-area-share[%]
    mineral_land_change_ha_2 = MCDNode(operation_selection='x * (1-y)', output_name='mineral-land-change[ha]')(input_table_1=land_change_ha, input_table_2=organic_area_share_percent_3)
    ...
    # Group by  Country, Years, land-use, climate-type, sub-climate-type (sum)
    land_management_ha_excluded_excluded_excluded = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_ha_excluded_excluded_excluded)
    # land-change[ha] (based on a 20 year lifespan) = land-management [ha] - land-management- lifetime-lagged [ha]
    land_change_ha_2 = MCDNode(operation_selection='x - y', output_name='land-change[ha]')(input_table_1=land_management_ha_excluded_excluded_excluded, input_table_2=land_management_lifetime_lagged_ha_excluded_excluded_excluded)
    # Group by  Country, Years, land-use, climate-type, sub-climate-type, soil-type (sum)
    land_management_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use', 'climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Sum')(df=land_management_ha_3)
    # land-change[ha] (based on a 20 year lifespan) = land-management [ha] - land-management- lifetime-lagged [ha]
    land_change_ha = MCDNode(operation_selection='x - y', output_name='land-change[ha]')(input_table_1=land_management_ha_2, input_table_2=land_management_lifetime_lagged_ha)
    # mineral-land-change[ha] = land-change[ha] * (1-organic-area-share[%]
    mineral_land_change_ha_3 = MCDNode(operation_selection='x * (1-y)', output_name='mineral-land-change[ha]')(input_table_1=land_change_ha, input_table_2=organic_area_share_percent_4)
    # CP soil-organic-c-stock (agr_soil-organic-stock) [tC/ha]
    agr_soil_organic_stock_tC_per_ha = ImportDataNode(trigram='agr', variable_name='agr_soil-organic-stock', variable_type='CP')()
    # Remove unused columns (data_type, source, module) (mean)
    agr_soil_organic_stock_tC_per_ha = GroupByDimensions(groupby_dimensions=['climate-type', 'sub-climate-type', 'soil-type'], aggregation_method='Mean')(df=agr_soil_organic_stock_tC_per_ha)
    # carbon-change[tC] = mineral-land-change[ha] * soil-organic-c-stock[tC/ha]
    carbon_change_tC_2 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=mineral_land_change_ha_3, input_table_2=agr_soil_organic_stock_tC_per_ha)
    ...
    soil_organic_c_stock_tC_per_ha = UseVariableNode(selected_variable='soil-organic-c-stock[tC/ha]')(input_table=agr_soil_organic_stock_tC_per_ha)
    # carbon-change[tC] = mineral-land-change[ha] * soil-organic-c-stock[tC/ha]
    carbon_change_tC_3 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=mineral_land_change_ha, input_table_2=soil_organic_c_stock_tC_per_ha)
    # carbon-change[tC] (replace) = carbon-change[tC] / 20
    ...
    # soil-organic-c-stock [tC/ha]
    soil_organic_c_stock_tC_per_ha = UseVariableNode(selected_variable='soil-organic-c-stock[tC/ha]')(input_table=soil_organic_c_stock_tC_per_ha)
    # carbon-change[tC] = mineral-land-change[ha] * soil-organic-c-stock[tC/ha]
    carbon_change_tC_4 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=mineral_land_change_ha_2, input_table_2=soil_organic_c_stock_tC_per_ha)
    # carbon-change[tC] (replace) = carbon-change[tC] / 20
    ...
    carbon_change_tC_5 = MCDNode(operation_selection='x * y', output_name='carbon-change[tC]')(input_table_1=land_change_ha_2, input_table_2=soil_organic_c_stock_tC_per_ha)
    # carbon-change[tC] (replace) = carbon-change[tC] / 20
    ...
    # carbon-change [tC]
    carbon_change_tC = UseVariableNode(selected_variable='carbon-change[tC]')(input_table=carbon_change_tC)
    # Group by  Country, Years, land-use (sum) 
    carbon_change_tC = GroupByDimensions(groupby_dimensions=['Country', 'Years', 'land-use'], aggregation_method='Sum')(df=carbon_change_tC)
    ...
    # Group by  Country, Years, (sum)  
    emissions_Mt_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=emissions_Mt)
    # Calibration emissions [Mt]
    emissions_Mt_3 = ImportDataNode(trigram='agr', variable_name='emissions', variable_type='Calibration')()
    # Keep emission-type = c-stock
    emissions_Mt_3 = emissions_Mt_3.loc[emissions_Mt_3['emission-type'].isin(['c-stock'])].copy()
    # Group by  Country, Years (sum)
    emissions_Mt_3 = GroupByDimensions(groupby_dimensions=['Years', 'Country'], aggregation_method='Sum')(df=emissions_Mt_3)
    # Apply Calibration on energy-demand[TWh]
    _, out_10017_2, _ = CalibrationNode(data_to_be_cal='emissions[Mt]', data_cal='emissions[Mt]')(input_table=emissions_Mt_2, cal_table=emissions_Mt_3)

    # Cal_rate for emissions[Mt]

    # cal_rate for emissions[Mt]
    cal_rate_emissions_Mt = UseVariableNode(selected_variable='cal_rate_emissions[Mt]')(input_table=out_10017_2)
    # Module = CALIBRATION
    cal_rate_emissions_Mt = ColumnFilterNode(pattern='^.*$')(df=cal_rate_emissions_Mt)
    # emissions[Mt] (replace) = emissions[Mt] * ca-rate
    emissions_Mt = MCDNode(operation_selection='x * y', output_name='emissions[Mt]')(input_table_1=emissions_Mt, input_table_2=out_10017_2)
    # gaes = CO2
    ...
    out_9520_1, _ = LagVariable(in_var='land-management[ha]')(df=land_management_ha)
    # land-change[ha] = land-management[ha] - land-management-lagged[ha]
    land_change_ha = MCDNode(operation_selection='x - y', output_name='land-change[ha]')(input_table_1=land_management_ha, input_table_2=out_9520_1)
    # land-change[%] = land-change[ha] / land-management[ha]
    land_change_percent = MCDNode(operation_selection='y / x', output_name='land-change[%]')(input_table_1=land_management_ha, input_table_2=land_change_ha)
    # Set 0
    land_change_percent = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=land_change_percent)
    ...
    # Group by Country, Years, (sum)
    land_management_ha_2 = GroupByDimensions(groupby_dimensions=['Country', 'Years'], aggregation_method='Sum')(df=land_management_ha)
    # land-management-share[ha/ha] = land-management[ha] (per land type) / land-management[ha] (sum over all land type)
    land_management_share_ha_per_ha = MCDNode(operation_selection='x / y', output_name='land-management-share[ha/ha]')(input_table_1=land_management_ha, input_table_2=land_management_ha_2)
    # Set 0
    land_management_share_ha_per_ha = MissingValueNode(DTS_DT_O=[['org.knime.core.data.def.IntCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.StringCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.DoNothingMissingCellHandlerFactory'], ['org.knime.core.data.def.DoubleCell', 'org.knime.base.node.preproc.pmml.missingval.handlers.FixedDoubleValueMissingCellHandlerFactory']], FixedValue='0.0')(df=land_management_share_ha_per_ha)
    ...
    out_9187_1 = AddTrigram()(module_name=module_name, df=out_9533_1)
    # Module = Pathway Explorer
    out_9187_1 = ColumnFilterNode(pattern='^.*$')(df=out_9187_1)
